#### GvG Embeddings Generator (GvG_EG) v1.3 ####
## Versão com configuração avançada de pré-processamento ##
...
# Importar a função de pré-processamento e as funções de gerenciamento de nomes de arquivos
...
# Opções de pré-processamento (configuração inicial)
preprocessing_options = {
    "remove_special_chars": True,     # Remover caracteres especiais
    "keep_separators": False,         # Manter separadores gráficos (vírgulas, pontos, etc)
    "remove_accents": True,           # Remover acentos
    "case": "lower",                  # Converter caso: "lower", "upper", ou "original"
    "remove_stopwords": True,         # Remover stopwords
    "lemmatize": True                 # Aplicar lematização
}
...
# Atualizar nome do arquivo de embeddings usando a função do módulo GvG_PP_v0
GvG_EMBEDDINGS_FILE = gvg_create_embedding_filename(EMBEDDINGS_PATH, EMBEDDING_MODEL_INDEX, preprocessing_options)
...
def selecionar_modelo():
    ...
    global EMBEDDING_MODEL_INDEX
    
    console.print(Panel("[bold magenta]SELEÇÃO DE MODELO DE EMBEDDING[/bold magenta]", expand=False))
    console.print("\nModelos disponíveis:")
    ...
    try:
        if choice.strip():
            ...
            if 0 <= idx < len(embedding_models):
                model_name = embedding_models[idx]["modelo"]
                EMBEDDING_MODEL_INDEX = EMBEDDING_MODELS_REVERSE.get(model_name, 0)
                return model_name
...
def configurar_preprocessamento():
    """Permite ao usuário configurar opções de pré-processamento de texto."""
    global preprocessing_options
    
    console.print(Panel("[bold magenta]CONFIGURAÇÃO DE PRÉ-PROCESSAMENTO DE TEXTO[/bold magenta]", expand=False))
    
    # 1. Configuração para remoção de caracteres especiais (pergunta separada)
    console.print("\n[cyan]1. Tratamento de caracteres especiais:[/cyan]")
    console.print("   1. Remover caracteres especiais")
    console.print("   2. Manter caracteres especiais")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if preprocessing_options['remove_special_chars'] else '2'}]: ")
    if choice.strip() == '1':
        preprocessing_options['remove_special_chars'] = True
    elif choice.strip() == '2':
        preprocessing_options['remove_special_chars'] = False
    
    # 2. Configuração para separadores (pergunta separada)
    console.print("\n[cyan]2. Tratamento de separadores gráficos (pontuação):[/cyan]")
    console.print("   1. Manter separadores gráficos (dois pontos, ponto e vírgula, vírgula, ponto, hifens, etc.)")
    console.print("   2. Remover separadores gráficos")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if preprocessing_options['keep_separators'] else '2'}]: ")
    if choice.strip() == '1':
        preprocessing_options['keep_separators'] = True
    elif choice.strip() == '2':
        preprocessing_options['keep_separators'] = False
    
    # 3. Configuração para remoção de acentos
    console.print("\n[cyan]3. Tratamento de acentos:[/cyan]")
    console.print("   1. Remover acentos")
    console.print("   2. Manter acentos")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if preprocessing_options['remove_accents'] else '2'}]: ")
    if choice.strip() == '1':
        preprocessing_options['remove_accents'] = True
    elif choice.strip() == '2':
        preprocessing_options['remove_accents'] = False
    
    # 4. Configuração de caixa (maiúscula/minúscula)
    console.print("\n[cyan]4. Tratamento de maiúsculas/minúsculas:[/cyan]")
    console.print("   1. Converter para minúsculas")
    console.print("   2. Converter para maiúsculas")
    console.print("   3. Manter como está no texto original")
    case_option = {'lower': '1', 'upper': '2', 'original': '3'}
    choice = input(f"   Escolha (1-3) [padrão: {case_option[preprocessing_options['case']]}]: ")
    if choice.strip() == '1':
        preprocessing_options['case'] = 'lower'
    elif choice.strip() == '2':
        preprocessing_options['case'] = 'upper'
    elif choice.strip() == '3':
        preprocessing_options['case'] = 'original'
    
    # 5. Configuração de stopwords
    console.print("\n[cyan]5. Tratamento de stopwords (palavras comuns como 'e', 'o', 'a', etc.):[/cyan]")
    console.print("   1. Remover stopwords")
    console.print("   2. Manter stopwords")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if preprocessing_options['remove_stopwords'] else '2'}]: ")
    if choice.strip() == '1':
        preprocessing_options['remove_stopwords'] = True
    elif choice.strip() == '2':
        preprocessing_options['remove_stopwords'] = False
    
    # 6. Configuração de lematização
    console.print("\n[cyan]6. Lematização (reduzir palavras à sua forma base):[/cyan]")
    console.print("   1. Aplicar lematização")
    console.print("   2. Não aplicar lematização")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if preprocessing_options['lemmatize'] else '2'}]: ")
    if choice.strip() == '1':
        preprocessing_options['lemmatize'] = True
    elif choice.strip() == '2':
        preprocessing_options['lemmatize'] = False
    
    # Resumo das configurações
    console.print("\n[green]Configurações de pré-processamento atualizadas:[/green]")
    console.print(f"- Remover caracteres especiais: {preprocessing_options['remove_special_chars']}")
    console.print(f"- Manter separadores gráficos: {preprocessing_options['keep_separators']}")
    console.print(f"- Remover acentos: {preprocessing_options['remove_accents']}")
    console.print(f"- Caixa de texto: {preprocessing_options['case']}")
    console.print(f"- Remover stopwords: {preprocessing_options['remove_stopwords']}")
    console.print(f"- Aplicar lematização: {preprocessing_options['lemmatize']}")
...
def processar_descricoes(df, options=None):
    """Pré-processa as descrições de contratações usando as opções configuradas."""
    if options is None:
        options = preprocessing_options
        
    console.print("[bold magenta]Processando descrições...[/bold magenta]")
    
    # Usar a função importada de GvG_PP_v0.py
    df['descricao_processada'] = df['descricaoCompleta'].apply(
        lambda x: gvg_pre_processing(
            x,
            remove_special_chars=options['remove_special_chars'],
            keep_separators=options['keep_separators'],
...
def processar_arquivo(excel_path, preproc_options=None):
    """Processa um arquivo Excel e atualiza a base de embeddings."""
    if preproc_options is None:
        preproc_options = preprocessing_options
    ...
    # Processar descrições com as opções configuradas
    df = processar_descricoes(df, preproc_options)
    ...
    with open(log_file, 'a') as f:
        ...
        f.write(f"Configuração de pré-processamento: {preprocessing_options}\n")
...
def processar_diretorio_new(preproc_options=None):
    """Processa todos os arquivos Excel no diretório NEW_PATH e pergunta se deseja movê-los para OLD_PATH."""
    if preproc_options is None:
        preproc_options = preprocessing_options
        
    console.print(Panel("[bold magenta]PROCESSAMENTO DE ARQUIVOS[/bold magenta]", expand=False))
    ...
    # Lista de arquivos processados para perguntar sobre movimentação depois
    ...
    for i, excel_file in enumerate(excel_files, 1):
        ...
        # Processar o arquivo com as opções configuradas
        novos, atualizados, total = processar_arquivo(excel_path, preproc_options)
    ...
    # Perguntar sobre a movimentação dos arquivos
    if arquivos_processados:
        console.print("\n[bold magenta]GESTÃO DE ARQUIVOS PROCESSADOS[/bold magenta]")
        
        # Mostrar resumo dos arquivos processados
        table = Table(show_header=True, header_style="bold cyan")
        table.add_column("#", style="dim", width=3)
        table.add_column("Arquivo", style="blue")
        table.add_column("Novos Registros", justify="right", style="green")
        table.add_column("Atualizados", justify="right", style="yellow")
        
        for i, arquivo in enumerate(arquivos_processados, 1):
            table.add_row(
                str(i), 
                arquivo["filename"], 
                str(arquivo["novos"]), 
                str(arquivo["atualizados"])
            )
        
        console.print(table)
        
        # Pergunta geral para todos os arquivos
        while True:
            resposta = input("\nDeseja mover os arquivos processados para o diretório OLD? (s/n): ").strip().lower()
            if resposta in ['s', 'sim', 'n', 'não', 'nao']:
                break
            console.print("[yellow]Por favor, responda 's' para sim ou 'n' para não.[/yellow]")
        
        if resposta in ['s', 'sim']:
            console.print("[cyan]Movendo arquivos para o diretório OLD...[/cyan]")
            
            for arquivo in arquivos_processados:
                excel_path = arquivo["path"]
                excel_file = arquivo["filename"]
                
                # Destino mantém o mesmo nome do arquivo original
                destino = os.path.join(OLD_PATH, excel_file)
                
                # Verificar se já existe um arquivo com o mesmo nome
                if os.path.exists(destino):
                    # Adicionar timestamp apenas se houver conflito
                ...
                try:
                    ...
                    console.print(f"[bold red]Erro ao mover arquivo {excel_file}: {str(e)}[/bold red]")
        else:
            console.print("[cyan]Os arquivos permanecerão no diretório NEW.[/cyan]")

def main():
    """Função principal que processa todos os arquivos em NEW_PATH."""
    console.print(Panel("[bold magenta]SISTEMA DE GERAÇÃO DE EMBEDDINGS PARA CONTRATAÇÕES v1.3[/bold magenta]", subtitle="[bold cyan]Com opções avançadas de pré-processamento[/bold cyan]"))
    
    # Selecionar modelo de embedding
    global EMBEDDING_MODEL, EMBEDDING_MODEL_INDEX, EMBEDDING_DIM, GvG_EMBEDDINGS_FILE
    ...
    global preprocessing_options
    preprocessing_options = configurar_preprocessamento()
    
    # Atualizar nome do arquivo de embeddings usando a função do módulo GvG_PP_v0
    GvG_EMBEDDINGS_FILE = gvg_create_embedding_filename(EMBEDDINGS_PATH, EMBEDDING_MODEL_INDEX, preprocessing_options)
    console.print(f"[bold cyan]Arquivo de embeddings: {os.path.basename(GvG_EMBEDDINGS_FILE)}[/bold cyan]\n")
    
    # Processar todos os arquivos em NEW_PATH com as opções configuradas
    processar_diretorio_new(preprocessing_options)

# Verificar se existe um embedding com o mesmo nome mas configurações diferentes
def verificar_embeddings_existentes():
    """Verifica se existem arquivos de embeddings com nomes semelhantes mas configurações diferentes."""
    console.print("[cyan]Verificando embeddings existentes...[/cyan]")
    
    # Buscar todos os arquivos de embeddings
    ...
    for filename in embedding_files:
        ...
        try:
            ...
            model_name, model_index, file_options = gvg_parse_embedding_filename(filepath)
            
            console.print(f"[cyan]Encontrado arquivo: {filename}[/cyan]")
            console.print(f"  Modelo: {model_name} (índice: {model_index})")
            console.print(f"  Opções de pré-processamento:")
            for key, value in file_options.items():
                console.print(f"    - {key}: {value}")
            console.print("")
            
        except Exception as e:
            console.print(f"[yellow]Não foi possível analisar o arquivo {filename}: {str(e)}[/yellow]")
    
    console.print("[green]Verificação de embeddings concluída.[/green]")

if __name__ == "__main__":
    # Opcionalmente verificar embeddings existentes
    # verificar_embeddings_existentes()
