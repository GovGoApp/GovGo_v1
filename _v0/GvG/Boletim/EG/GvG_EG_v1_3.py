...
# Importar a função de pré-processamento do módulo separado
from GvG_PP_v0 import gvg_pre_processing
...
# Gerar nome do arquivo de embeddings baseado no modelo e opções de pré-processamento
def update_embedding_filename():
    """Atualiza o nome do arquivo de embeddings com base no modelo e opções de pré-processamento."""
    # Criar código para as opções de pré-processamento
    preproc_code = (
        f"{'A' if preprocessing_options['remove_accents'] else 'a'}"
        f"{'S' if preprocessing_options['remove_special_chars'] else 's'}"
        f"{'G' if preprocessing_options['keep_separators'] else 'g'}"
        f"{'L' if preprocessing_options['case'] == 'lower' else 'U' if preprocessing_options['case'] == 'upper' else 'O'}"
        f"{'W' if preprocessing_options['remove_stopwords'] else 'w'}"
        f"{'Z' if preprocessing_options['lemmatize'] else 'z'}"
    )
    
    return EMBEDDINGS_PATH + f"GvG_embeddings_{EMBEDDING_MODEL}_{preproc_code}.pkl"

# Atualizar nome do arquivo de embeddings
GvG_EMBEDDINGS_FILE = update_embedding_filename()
...
def configurar_preprocessamento():
    ...
    # 1. Configuração de caracteres especiais e separadores
    console.print("\n[cyan]1. Caracteres especiais e separadores:[/cyan]")
    console.print("   1. Remover todos os caracteres especiais")
    console.print("   2. Manter separadores gráficos (dois pontos, ponto e vírgula, vírgula, ponto, hifens, etc.)")
    choice = input(f"   Escolha (1-2) [padrão: {'1' if not preprocessing_options['keep_separators'] else '2'}]: ")
    if choice.strip() == '1':
    ...
    # 2. Configuração para remoção de acentos
    console.print("\n[cyan]2. Tratamento de acentos:[/cyan]")
    ...
    # 3. Configuração de caixa (maiúscula/minúscula)
    console.print("\n[cyan]3. Tratamento de maiúsculas/minúsculas:[/cyan]")
    ...
    # 4. Configuração de stopwords
    console.print("\n[cyan]4. Tratamento de stopwords (palavras comuns como 'e', 'o', 'a', etc.):[/cyan]")
    ...
    # 5. Configuração de lematização
    console.print("\n[cyan]5. Lematização (reduzir palavras à sua forma base):[/cyan]")
...
def processar_descricoes(df, options=None):
    ...
    # Usar a função importada de text_preprocessor.py
    df['descricao_processada'] = df['descricaoCompleta'].apply(
        lambda x: gvg_pre_processing( #pre-processamento de texto
            x,
