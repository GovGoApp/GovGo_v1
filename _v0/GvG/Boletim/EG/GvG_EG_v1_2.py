...
def preprocess_text(text, options=None):
    ...
    if options['remove_special_chars']:
        if options['keep_separators']:
            # Definir padrão regex baseado na configuração de caixa
            if options['case'] == 'lower':
...
def processar_descricoes(df, options=None):
    ...
    df['descricao_processada'] = df['descricaoCompleta'].apply(lambda x: preprocess_text(x, options))
