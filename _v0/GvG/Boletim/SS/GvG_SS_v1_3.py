...
def search_similar_items(query_text, model_name, preproc_options, embeddings_dict, faiss_index, keys, similarity_method=0, top_n=TOP_N):
    """Busca itens similares à consulta usando diferentes métodos de similaridade."""
    ...
    # Realizar busca
    distances, indices = faiss_index.search(query_embedding_np, top_n)
    
    # Processar resultados
    results = []
    
    # Converter scores conforme o método
    if similarity_method == 1:
...
def main():
    console.print(Panel(
        ...
        title="[bold magenta]SISTEMA DE BUSCA POR SIMILARIDADE DE CONTRATAÇÕES v1.3[/bold magenta]",
        subtitle="[bold cyan]Métodos de similaridade configuráveis[/bold cyan]",
        expand=False
    ))
    ...
    while True:
        ...
        console.print("[cyan]3[/cyan] - Exportar últimos resultados para Excel")
