...
app.layout = html.Div([
    ...
    dcc.Store(id='embedding-cache', data={'model': SELECTED_MODEL, 'loaded': False, 'status_message': 'Aguardando carregamento do modelo.', 'status_style': {'color': '#D32F2F', 'fontWeight': 'bold'}}), # Adicionado status inicial
...
# Nova função auxiliar para carregar embeddings
def load_fixed_model_embeddings():
    global EMBEDDINGS_DICT, FAISS_INDEX, INDEX_KEYS, SELECTED_MODEL, EMBEDDING_DIM
    
    debug_print(f"Tentando carregar embeddings para o modelo fixo: {SELECTED_MODEL}")
    
    # Garante que a dimensão global está correta para o modelo fixo
    current_model_dim = get_embedding_dimension(SELECTED_MODEL)
    if EMBEDDING_DIM != current_model_dim:
        EMBEDDING_DIM = current_model_dim
        debug_print(f"Dimensão do embedding atualizada para: {EMBEDDING_DIM}")

    try:
        embeddings_dict_loaded = load_embeddings(SELECTED_MODEL)
        if not embeddings_dict_loaded:
            error_msg = f"Falha ao carregar arquivo de embeddings para {SELECTED_MODEL}."
            debug_print(error_msg)
            return False, error_msg, {'color': '#D32F2F', 'fontWeight': 'bold'}

        faiss_index_loaded, keys_loaded = create_faiss_index(embeddings_dict_loaded, EMBEDDING_DIM)
        
        EMBEDDINGS_DICT = embeddings_dict_loaded
        FAISS_INDEX = faiss_index_loaded
        INDEX_KEYS = keys_loaded
        
        success_msg = f"Modelo {SELECTED_MODEL} carregado ({len(EMBEDDINGS_DICT)} itens)"
        debug_print(success_msg)
        return True, success_msg, {'color': '#4CAF50', 'fontWeight': 'bold'}
    except Exception as e:
        error_msg = f"Erro crítico ao carregar embeddings para {SELECTED_MODEL}: {e}"
        debug_print(error_msg)
        debug_print(traceback.format_exc())
        return False, f"Erro: {str(e)[:50]}...", {'color': '#D32F2F', 'fontWeight': 'bold'}
...
@app.callback(
    ...
    Output('embedding-cache', 'data'), # Adicionar output para atualizar o cache
    Output('search-status', 'children'), # Adicionar output para status
    Output('search-status', 'style'),   # Adicionar output para estilo do status
...
def process_search_query(n_clicks, query_text, history, results, errors, counter, embedding_cache_data):
    ctx = dash.callback_context
    triggered_id = ctx.triggered[0]['prop_id'].split('.')[0]

    # Se o gatilho não for o botão ou n_clicks for None (inicial), não faz nada.
    if not n_clicks or triggered_id != 'submit-button-content':
        raise PreventUpdate

    if not query_text or not query_text.strip():
        # Se o texto da consulta estiver vazio, reseta o estado de processamento e retorna
        # Mantém o status atual do embedding-cache
        return history, results, errors, counter, False, embedding_cache_data, embedding_cache_data.get('status_message', "Pronto"), embedding_cache_data.get('status_style', {'color': '#4CAF50', 'fontWeight': 'bold'})

    current_status_message = embedding_cache_data.get('status_message')
    current_status_style = embedding_cache_data.get('status_style')

    # Verificar e carregar embeddings se necessário
    if not embedding_cache_data.get('loaded', False) or EMBEDDINGS_DICT is None:
        debug_print("Embeddings não carregados, tentando carregar...")
        loaded_successfully, status_msg, status_style = load_fixed_model_embeddings()
        embedding_cache_data['loaded'] = loaded_successfully
        embedding_cache_data['status_message'] = status_msg
        embedding_cache_data['status_style'] = status_style
        current_status_message = status_msg # Atualiza para uso imediato
        current_status_style = status_style

        if not loaded_successfully:
            new_error = {
                'id': str(counter),
                'query': query_text,
                'message': "Falha ao carregar modelo de embeddings.",
                'details': status_msg,
                'timestamp': datetime.now().isoformat()
            }
            updated_errors = errors + [new_error]
            new_history_item = {
                'id': str(counter), 'query': query_text, 'timestamp': datetime.now().isoformat(),
                'error': "Modelo não carregado", 'count': 0
            }
            updated_history = history + [new_history_item]
            return updated_history, results, updated_errors, counter + 1, False, embedding_cache_data, current_status_message, current_status_style
    
    # Se chegou aqui, embeddings devem estar carregados (ou houve tentativa)
    if not EMBEDDINGS_DICT or not FAISS_INDEX: # Dupla verificação
        # Este caso não deveria acontecer se load_fixed_model_embeddings foi chamado e teve sucesso
        # Mas como precaução:
        error_msg_critical = "Estado inconsistente: Dicionário de embeddings ou índice FAISS não disponível após tentativa de carregamento."
        debug_print(error_msg_critical)
        new_error = {
            'id': str(counter), 'query': query_text, 'message': error_msg_critical,
            'details': "Verifique os logs do servidor.", 'timestamp': datetime.now().isoformat()
        }
        updated_errors = errors + [new_error]
        new_history_item = {
            'id': str(counter), 'query': query_text, 'timestamp': datetime.now().isoformat(),
            'error': "Erro interno no modelo", 'count': 0
        }
        updated_history = history + [new_history_item]
        embedding_cache_data['status_message'] = "Erro no modelo."
        embedding_cache_data['status_style'] = {'color': '#D32F2F', 'fontWeight': 'bold'}
        return updated_history, results, updated_errors, counter + 1, False, embedding_cache_data, "Erro no modelo.", {'color': '#D32F2F', 'fontWeight': 'bold'}


    try:
        # Buscar itens similares
        search_results_data, confidence, processed_query = search_similar_items(
            query_text, 
            SELECTED_MODEL, 
            EMBEDDINGS_DICT, 
            FAISS_INDEX, 
        ...
        new_history_item = {
            'id': str(counter), 'query': query_text, 'processed_query': processed_query,
            'timestamp': datetime.now().isoformat(), 'confidence': confidence, 'count': len(search_results_data)
        }
        updated_history = history + [new_history_item]
        
        new_result_item = {
            'id': str(counter), 'query': query_text, 'processed_query': processed_query,
            'data': search_results_data, 'confidence': confidence, 'timestamp': datetime.now().isoformat()
        }
        updated_results = results + [new_result_item]
        
        return updated_history, updated_results, errors, counter + 1, False, embedding_cache_data, current_status_message, current_status_style
        
    except Exception as e:
        error_msg = str(e)
        stack_trace = traceback.format_exc()
        debug_print(f"Erro em process_search_query: {error_msg}\n{stack_trace}")
        
        new_history_item = {
            'id': str(counter), 'query': query_text, 'timestamp': datetime.now().isoformat(),
            'error': error_msg, 'count': 0
        }
        updated_history = history + [new_history_item]
        
        new_error = {
            'id': str(counter), 'query': query_text, 'message': error_msg,
            'details': stack_trace, 'timestamp': datetime.now().isoformat()
        }
        updated_errors = errors + [new_error]
...
@app.callback(
    ...
    Input('processing-state', 'data'), # Único Input
    State('submit-button-content', 'style') # State para o estilo atual
)
def update_button_state(is_processing, current_style):
    # Este callback é acionado sempre que 'processing-state' muda.
    # prevent_initial_call não é especificado, então é False por padrão, o que é bom para o estado inicial.
    if is_processing:
        ...
        return [
            ...
            **current_style,
            'opacity': '0.8', # Mantém outros estilos do botão
            'backgroundColor': '#FF5722', # Garante a cor de fundo
            'color': 'white', # Garante a cor do spinner (se não for sobrescrita pelo ícone)
        }
    else:
...
def set_processing_state(n_clicks, query_text):
    # n_clicks é o gatilho principal.
    # Se n_clicks for None (estado inicial) ou 0, prevent_initial_call=True já cuida disso.
    # Se o botão for clicado sem texto, não deve processar.
    if not query_text or not query_text.strip():
        # Não muda o estado de processamento se não houver texto.
        # process_search_query também lidará com isso e definirá processing-state para False.
        raise PreventUpdate 
        
    debug_print(f"Botão clicado (n_clicks: {n_clicks}). Definindo processing-state para True.")
...
app.clientside_callback(
    """
    function(n_clicks_button) { // O input aqui é apenas para registrar o callback, não usamos o valor.
        const queryInput = document.getElementById('query-input');
        const submitButton = document.getElementById('submit-button-content');

        if (queryInput && submitButton) {
            if (!window.enterListenerAttachedToQueryInput) { // Evita adicionar múltiplos listeners
                queryInput.addEventListener('keydown', function(e) {
                    if (e.key === 'Enter' && !e.shiftKey) { // Enter, mas não Shift+Enter
                        e.preventDefault(); // Previne nova linha no textarea
                        if (!submitButton.disabled) {
                            submitButton.click(); // Simula clique no botão
                        }
                    }
                });
                window.enterListenerAttachedToQueryInput = true;
            }
        }
        return window.dash_clientside.no_update; // Não atualiza nenhum output do Dash
    }
    """,
    Output('query-input', 'placeholder'), # Output fictício, apenas para registrar o callback. Pode ser qualquer propriedade de qualquer componente.
    Input('submit-button-content', 'n_clicks'), # Input para garantir que o script seja registrado.
    prevent_initial_call=False # Executa na carga para adicionar o listener
)
