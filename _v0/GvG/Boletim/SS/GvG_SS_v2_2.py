...
input_container = html.Div([
    dcc.Textarea(
        ...
        # Removido o n_submit que estava causando o erro
    ),
    html.Button(
        html.Div([
        ...
        n_clicks=0,  # Inicializar explicitamente
        style={
            'backgroundColor': '#FF5722',
            'color': 'white',
            'border': 'none',
    ...
    # Adicionamos um div oculto para armazenar eventos de teclado
    html.Div(id='textarea-key-events', style={'display': 'none'})
], style=styles['input_container'])

# Adicionamos JavaScript para capturar a tecla Enter
app.clientside_callback(
    """
    function(n_clicks, value) {
        // Capturador de eventos para o textarea
    ...
    """,
    Output('textarea-key-events', 'children'),
...
# Modificar o callback de carregamento do modelo para evitar conflitos
@app.callback(
...
# Processar consulta e buscar resultados
@app.callback(
    Output('query-history-store', 'data'),
    Output('results-store', 'data'),
    Output('errors-store', 'data'),
    Output('counter-store', 'data'),
    Output('processing-state', 'data'),  # Garante que processamento termine com False
    Input('submit-button-content', 'n_clicks'),  # Apenas o botão agora (Enter vai acionar o botão)
    [State('query-input', 'value'),
...
def process_search_query(btn_clicks, query_text, history, results, errors, counter, cache, is_processing):
    # Verificar se já está processando (evitar chamadas duplicadas)
    if is_processing:
        raise PreventUpdate
        
    # Verificar se tem texto para pesquisar
    ...
    # Retorna com processamento concluído (False)
    return updated_history, updated_results, errors, counter + 1, False

# 3. Separar o callback para definir o estado de processamento ANTES da execução
@app.callback(
    Output('processing-state', 'data'),
    Input('submit-button-content', 'n_clicks'),  # Apenas o botão agora
    [State('query-input', 'value'),
     State('processing-state', 'data')],
    prevent_initial_call=True
)
def set_processing_state(btn_clicks, query_text, current_state):
    # Verificar se foi acionado
    if not callback_context.triggered:
        raise PreventUpdate
    
    trigger = callback_context.triggered[0]['prop_id'].split('.')[0]
    if trigger != 'submit-button-content' and trigger != 'query-input':
        raise PreventUpdate
    
    # Verificar se tem algum texto para buscar
    if not query_text or not query_text.strip():
        raise PreventUpdate
