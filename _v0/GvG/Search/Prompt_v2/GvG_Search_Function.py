...
# Export helpers
# --------------------------------------------------------------------------------------------------
def _generate_export_filename(query: str, search_type: int, approach: int, relevance: int, order: int, intelligent_enabled: bool, output_dir: str, extension: str) -> str:
    try:
        try:
...
def _export_json(results: List[dict], query: str, params: dict, output_dir: str) -> str:
    status = get_intelligent_status()
    fn = _generate_export_filename(query, params['search'], params['approach'], params['relevance'], params['order'], status['intelligent_processing'], output_dir, 'json')
    rows = []
    for r in results:
        d = r.get('details', {})
        rows.append({
    ...
    payload = {
        'metadata': {
            'query': query,
            'search_type': SEARCH_TYPES[params['search']]['name'],
            'approach': SEARCH_APPROACHES[params['approach']]['name'],
            'relevance_level': params['relevance'],
            'order': params['order'],
...
def _export_xlsx(results: List[dict], query: str, params: dict, output_dir: str) -> str:
    fn = _generate_export_filename(query, params['search'], params['approach'], params['relevance'], params['order'], get_intelligent_status()['intelligent_processing'], output_dir, 'xlsx')
    rows = []
    for r in results:
        d = r.get('details', {})
        rows.append({
            'Rank': r.get('rank'), 'ID': r.get('id'), 'Similaridade': r.get('similarity'),
...
def _export_pdf(results: List[dict], query: str, params: dict, output_dir: str) -> str | None:  # pragma: no cover
    if not REPORTLAB_AVAILABLE:
        return None
    fn = _generate_export_filename(query, params['search'], params['approach'], params['relevance'], params['order'], get_intelligent_status()['intelligent_processing'], output_dir, 'pdf')
    doc = SimpleDocTemplate(fn, pagesize=A4, rightMargin=45,leftMargin=45,topMargin=54,bottomMargin=54)
    ...
    elements.append(Paragraph(f"Tipo: {SEARCH_TYPES[params['search']]['name']} | Abordagem: {SEARCH_APPROACHES[params['approach']]['name']} | Relevância: {RELEVANCE_LEVELS[params['relevance']]['name']}", normal))
    elements.append(Paragraph(f'Exportado em: {datetime.now().strftime("%d/%m/%Y %H:%M")}', normal))
    elements.append(Spacer(1, 0.2*inch))
    table_data = [["Rank","Unidade","Local","Similarity","Valor (R$)","Enc."]]
    for r in results:
        d = r.get('details', {})
        ...
        table_data.append([str(r.get('rank')), unidade[:30], local[:22], f"{r.get('similarity',0):.4f}", valor, str(data_enc)])
