"""
gvg_categories.py
Módulo especializado para funcionalidades de CATEGORIZAÇÃO e abordagens
"Correspondência" e "Filtro" replicando lógica do Terminal v9.

Objetivos:
 - Extrair ao máximo a lógica pesada do arquivo de Terminal
 - Reutilizar funções de busca existentes (semantic/keyword/hybrid)
 - Fornecer API simples para o terminal: obter categorias e executar buscas

Funções principais expostas:
 - get_top_categories_for_query(query_text, top_n, use_negation, search_type)
 - correspondence_search(query_text, top_categories, limit, filter_expired)
 - category_filtered_search(query_text, search_type, top_categories, limit, filter_expired, expanded_factor=3)

As funções retornam sempre (results, confidence, extra_info) onde extra_info
contém metadados úteis (ex: universe_size, filter_counts, etc.).
"""
