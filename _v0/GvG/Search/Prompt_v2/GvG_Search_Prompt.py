...
# EXPORTAÇÃO
# =====================================================================================
def generate_export_filename(query: str, search_type: int, approach: int, relevance: int, order: int, intelligent_enabled: bool, output_dir: str, extension: str) -> str:
    try:
        # Usar apenas termos positivos se negation ativa (replica v9/v2)
        try:
...
def export_results_json(results: List[dict], query: str, params, output_dir: str):
    status = get_intelligent_status()
    filename = generate_export_filename(query, params.search, params.approach, params.relevance, params.order, status['intelligent_processing'], output_dir, 'json')
    data_rows = []
    for r in results:
        d = r.get('details', {})
        data_rows.append({
...
def export_results_excel(results: List[dict], query: str, params, output_dir: str):
    filename = generate_export_filename(query, params.search, params.approach, params.relevance, params.order, get_intelligent_status()['intelligent_processing'], output_dir, 'xlsx')
    rows = []
    for r in results:
        d = r.get('details', {})
        row = {
...
def export_results_pdf(results: List[dict], query: str, params, output_dir: str):  # pragma: no cover (visual)
    if not REPORTLAB_AVAILABLE:
        return None
    filename = generate_export_filename(query, params.search, params.approach, params.relevance, params.order, get_intelligent_status()['intelligent_processing'], output_dir, 'pdf')
    ...
    try:
        pos, neg = extract_pos_neg_terms(query)
        display_query = pos.strip() if pos.strip() else query
    except Exception:
        display_query = query
    elements.append(Paragraph(f'BUSCA: "{display_query.upper()}"', title_style))
