"""
GvG_SP_Search_Terminal_v4.py
Sistema Avançado de Busca PNCP - Versão 4.0
...
MAX_RESULTS = 20    # Número máximo de resultados que serão retornados
...
def format_date(date_str):
    ...
    try:
        # Converte YYYY-MM-DD para DD/MM/YYYY usando simples manipulação de string
...
def get_date_color(date_str):
    """Retorna a cor da data baseada na urgência"""
    if not date_str or date_str == "N/A":
        return "dim"
    
    try:
        # Converter YYYY-MM-DD para objeto date
        year, month, day = date_str.split('-')
        ...
        if delta < 0:  # Data já passou
            return "dim"
        elif delta <= 7:  # Até 7 dias (D0 a D7)
            return "bold red"
        elif delta <= 14:  # Entre 8 e 14 dias (D8 a D14)
            return "orange3"
        elif delta <= 30:  # Entre 15 e 30 dias (D15 a D30)
            return "yellow"
        else:  # Mais de 30 dias (D31+)
            return "green"
    except:
        return "dim"  # Cor padrão para datas inválidas
...
def display_results(results, confidence, query, search_type_id):
    ...
    console.print("\n[bold magenta]Resumo dos Resultados[/bold magenta]")
    table = Table(show_header=True, header_style="bold magenta")
    table.add_column("Rank", style="dim", width=6, justify="center")
    table.add_column("Unidade", style="cyan", width=40)  # Nova coluna "Unidade"
    ...
    for result in results:
        ...
        # Formatar data com cor
        data_encerramento_orig = details.get("dataencerramentoproposta", "N/A") if details else "N/A"
        date_color = get_date_color(data_encerramento_orig)
        data_encerramento = format_date(data_encerramento_orig)
        
        # Preparar informações de unidade e local
        ...
        table.add_row(
            ...
            valor,
            f"[{date_color}]{data_encerramento}[/{date_color}]"  # Data colorida e formatada
        )
    ...
    for result in results:
        ...
        # Formatar datas para exibição no painel
        data_abertura_orig = details.get('dataaberturaproposta', 'N/A')
        data_encerramento_orig = details.get('dataencerramentoproposta', 'N/A')
        data_abertura = format_date(data_abertura_orig)
        data_encerramento = format_date(data_encerramento_orig)
        date_color = get_date_color(data_encerramento_orig)
        ...
        content = [
            ...
            f"[bold cyan]Datas:[/bold cyan] Abertura: {data_abertura} | Encerramento: [{date_color}]{data_encerramento}[/{date_color}]",
            f"[bold cyan]Descrição:[/bold cyan] {descricao[:500]}..."
        ]
...
def export_results_to_pdf(results, query, search_type_id):
    ...
    try:
        ...
        # Definir cores para datas
        date_colors = {
            "red": colors.red,
            "orange3": colors.orangered,
            "yellow": colors.gold,
            "green": colors.green,
            "dim": colors.gray
        }

        # Definir cores hexadecimais diretamente
        hex_colors = {
            "red": "#FF0000",
            "orange3": "#FF6600",
            "yellow": "#FFCC00",
        ...
        # Cores para a tabela com datas coloridas
        table_styles = [
        ...
        # Adicionar linhas à tabela e definir estilos de cores para datas
        row_idx = 1
        for result in sorted_results:
            ...
            # Formatar data com cor
            data_encerramento_orig = details.get("dataencerramentoproposta", "N/A")
            color_key = get_date_color(data_encerramento_orig).replace("bold ", "")  # Determina cor com data original
            data_encerramento = format_date(data_encerramento_orig)  # Formata para exibição
            
            # Adicionar estilo específico para esta célula de data

            # Na função export_results_to_pdf, na parte onde gera cores para as datas
            if color_key in hex_colors and data_encerramento != "N/A":
                color_hex = hex_colors[color_key]
                data_enc_colorida = f"<font color='{color_hex}'><b>{data_encerramento}</b></font>"
            ...
            table_data.append([
                ...
                f"{result['similarity']:.4f}",
                valor,
                data_encerramento  # Data formatada
            ])
            row_idx += 1
        ...
        for result in sorted_results:
            ...
            orgao = details.get('orgaoentidade_razaosocial', 'Órgão não informado')
            elements.append(Paragraph(f"{result['rank']}. {orgao}", orgao_style))
            
            # Subtítulo do card: Unidade
            unidade = details.get('unidadeorgao_nomeunidade', 'Unidade não informada')
            elements.append(Paragraph(f"{unidade}", unidade_style))
            
            # Conteúdo do card
            elements.append(Paragraph(f"<b>ID:</b> {result['id']}", normal_style))
            elements.append(Paragraph(f"<b>Similaridade:</b> {result['similarity']:.4f}", normal_style))
            elements.append(Paragraph(f"<b>Local:</b> {details.get('unidadeorgao_municipionome', 'N/A')}/{details.get('unidadeorgao_ufsigla', 'N/A')}", normal_style))
            elements.append(Paragraph(f"<b>Valor:</b> {format_currency(details.get('valortotalestimado', 0))}", normal_style))
            
            # Data de encerramento com cor
            data_abertura_orig = details.get('dataaberturaproposta', 'N/A')
            data_encerramento_orig = details.get('dataencerramentoproposta', 'N/A')
            color_key = get_date_color(data_encerramento_orig).replace("bold ", "")  # Cor com data original
            data_abertura = format_date(data_abertura_orig)  # Formata para exibição
            data_encerramento = format_date(data_encerramento_orig)  # Formata para exibição
            
            # Usar hex_colors diretamente em vez de tentar converter com toHex
            if color_key in hex_colors and data_encerramento != "N/A":
                color_hex = hex_colors[color_key]
                data_enc_colorida = f"<font color='{color_hex}'><b>{data_encerramento}</b></font>"
                elements.append(Paragraph(f"<b>Datas:</b> Abertura: {data_abertura} | Encerramento: {data_enc_colorida}", normal_style))
            else:
                elements.append(Paragraph(f"<b>Datas:</b> Abertura: {data_abertura} | Encerramento: {data_encerramento}", normal_style))
...
def display_menu():
    ...
    if last_results:
        console.print("[cyan]4[/cyan] - Exportar últimos resultados")
        console.print("[cyan]5[/cyan] - Ver documentos de um processo específico")
        console.print("[cyan]6[/cyan] - Gerar palavras-chave de um processo específico")
        console.print("[cyan]7[/cyan] - Encerrar o programa")
        console.print(f"\n[dim]Última busca: \"{last_query}\" - {len(last_results)} resultados encontrados[/dim]")
    else:
        console.print("[cyan]4[/cyan] - Encerrar o programa")
