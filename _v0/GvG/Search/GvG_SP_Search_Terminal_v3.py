...
def display_results(results, confidence, query, search_type_id):
    ...
    # Aplicar filtro de data de encerramento, se ativado
    if filter_expired:
        ...
        for result in results:
            ...
            # Obter e converter a data de encerramento
            enc_date_str = details.get("dataencerramentoproposta")
            if enc_date_str and isinstance(enc_date_str, str):
                try:
                    enc_date = datetime.strptime(enc_date_str, "%Y-%m-%d").date()
                    if enc_date >= today:
                        filtered_results.append(result)
                    else:
                        filtered_out += 1
                except (ValueError, TypeError):
                    # Se a data for inválida, manter o resultado
                    filtered_results.append(result)
            else:
                # Se não tiver data, manter o resultado
                filtered_results.append(result)
        
        results = filtered_results
        filter_message = f"[yellow]Filtro ativo: {filtered_out} contratações encerradas foram ocultadas[/yellow]"
    else:
