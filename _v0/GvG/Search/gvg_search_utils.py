...
def semantic_search(query_text, limit=MAX_RESULTS, min_results=MIN_RESULTS, filter_expired=DEFAULT_FILTER_EXPIRED):
...
def hybrid_search(query_text, limit=MAX_RESULTS, min_results=MIN_RESULTS, semantic_weight=SEMANTIC_WEIGHT, filter_expired=DEFAULT_FILTER_EXPIRED):
...
def summarize_document(doc_url, max_tokens=500):
    """
    Sumariza um documento usando OpenAI GPT baseado na URL fornecida.
    
    Args:
        doc_url (str): URL do documento para sumarizar
        max_tokens (int): Número máximo de tokens para o resumo (padrão: 500)
        
    Returns:
        str: Resumo do documento ou mensagem de erro
    """
    ...
    prompt = f"""
Por favor, acesse e leia o conteúdo do documento disponível no seguinte link: {doc_url}
Resuma o conteúdo detalhadamente em tópicos e parágrafos, destacando os principais pontos, objetivos, especificações técnicas e critérios relevantes.
...
"""
    ...
    try:
        completion = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Você é um assistente especializado em sumarizar documentos técnicos e editais de licitação."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.3,
        ...
        print(f"Erro ao sumarizar documento: {e}")
