...
# Importar funções de busca (agora de gvg_search_utils_v2)
try:
    ...
    print("ERRO: Não foi possível importar as funções de busca de gvg_search_utils_v2.")
    sys.exit(1)
...
def configure_system():
    ...
    while True:
        ...
        if choice == "1":
            ...
            console.print("\n[bold yellow]🔧 Processador de Documentos[/bold yellow]")
            console.print(f"[blue]Atual: {DOCUMENT_PROCESSOR_VERSION}[/blue]")
            console.print("\n[bold cyan]Versões disponíveis:[/bold cyan]")
            console.print("[green]• v2 (MarkItDown)[/green] - Rápido, compatível, integração OpenAI")
            console.print("[green]• v3 (Docling)[/green] - Melhor para tabelas, ZIP, estruturas complexas")
            
            if Confirm.ask(f"\nDeseja alternar para {'v2 (MarkItDown)' if USE_DOCLING_V3 else 'v3 (Docling)'}?"):
...
def perform_search(query, search_type, search_approach):
    ...
    try:
        ...
        if search_approach in [2, 3]:  # Correspondência ou Filtro de categoria
            ...
            # Usar processed_query para manter consistência com abordagem direta e negation embeddings
...
def display_menu():
    ...
    console.print(f"5. Prompt Negativo: [bold]{'✓ Ativo' if use_negation_embeddings else '✗ Inativo'}[/bold]")
    console.print("6. Configurações do sistema")
    console.print(f"7. Processador de documentos: [bold cyan]{DOCUMENT_PROCESSOR_VERSION}[/bold cyan]")
    
    # Opções contextuais baseadas em resultados disponíveis
    if last_results:
        console.print("\n[bold yellow]Opções de resultados:[/bold yellow]")
        console.print("8. 📄 Ver documentos de um processo")
        console.print("9. 🔤 Gerar palavras-chave de um processo") 
        console.print("10. 📊 Exportar resultados")
    else:
...
def main():
    ...
    console.print(f"[blue]📄 Processador de documentos ativo: {DOCUMENT_PROCESSOR_VERSION}[/blue]")
    
    # Mostrar informações sobre o processador atual
    if USE_DOCLING_V3:
        console.print("[dim]• Docling: Melhor extração de tabelas, suporte a ZIP[/dim]")
    else:
        console.print("[dim]• MarkItDown: Mais rápido, integração OpenAI nativa[/dim]")
    ...
    while True:
        ...
        elif user_input == "9" and last_results:
            try:
                process_num = int(Prompt.ask(f"Número do processo (1-{len(last_results)})"))
        ...
        elif user_input == "10" and last_results:
