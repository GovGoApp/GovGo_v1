...
class MyVectorizer(TfidfVectorizer):
    def __init__(self, *args, **kwargs):
        ...
        self._tfidf = TfidfTransformer(norm=self.norm, smooth_idf=self.smooth_idf)
...
# Initialize the tfidf transformer manually and plug in the IDFs.
from sklearn.feature_extraction.text import TfidfTransformer
vectorizer._tfidf = TfidfTransformer(norm='l2', smooth_idf=True)
vectorizer._tfidf._idf_diag = sp.spdiags(idfs,
                                         diags=0,
                                         m=len(idfs),
                                         n=len(idfs))

# --- Modificação na inicialização do classificador ---
clf = linear_model.SGDClassifier(loss='modified_huber',
                                 penalty='l2',
                                 alpha=0.0001,
                                 fit_intercept=True,
                                 max_iter=60,  # Alterado de n_iter para max_iter
                                 shuffle=True,
                                 random_state=None,
                                 n_jobs=4,
...
print("Chaves disponíveis:", store.keys())
store.close()
