...
# Adicionamos o parâmetro start_page para retomar a coleta a partir de uma página específica
def fetch_month_data(data_inicial, data_final, start_page=1):
    ...
    safe_print(f"Iniciando a busca para o período {data_inicial} a {data_final} - Página {start_page}")
    ...
    safe_print(f"Página {start_page} de {total_paginas} concluída.")
    if total_paginas > start_page:
        with tqdm(total=total_paginas - start_page, desc="Progresso das páginas", ncols=100, leave=False) as pbar:
            for page in range(start_page + 1, total_paginas + 1):
                params["pagina"] = page
                response = requests.get(base_url, params=params)
                if response.status_code != 200:
...
def process_month(year, month, path, start_page=1):
    ...
    monthly_data = fetch_month_data(data_inicial_str, data_final_str, start_page=start_page)
...
def main():
    # Neste exemplo, processamos somente o mês 2025-01.
    # Para retomar a partir de uma página específica, passe start_page diferente de 1.
    meses = [(2025, 2)]
    start_page = 1243
    ...
    max_workers = 5  # ajuste conforme seus recursos
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Neste exemplo, para o mês 2025-01, vamos retomar a partir da página 1148.
        futures = [executor.submit(process_month, year, month, PATH, start_page) for year, month in meses]
