...
FILE_OUT = "OBJETOS_CONTRATOS_PNCP.xlsx"                 # Arquivo de saída
...
for sheet in tqdm(xls.sheet_names, desc="Anos"):
    ...
    # Lê as colunas necessárias para a agregação
    ...
    df_group = df_sheet.groupby(
        ["objetoContrato", "tipoContrato.id", "tipoContrato.nome", 
         "categoriaProcesso.id", "categoriaProcesso.nome", "unidadeOrgao.ufSigla"],
        as_index=False
    ).agg(
    ...
    if df_final is None:
        ...
        df_final = pd.merge(
            df_final, df_group, 
            on=["objetoContrato", "tipoContrato.id", "tipoContrato.nome", 
                "categoriaProcesso.id", "categoriaProcesso.nome", "unidadeOrgao.ufSigla"],
            how="outer"
        )
...
# Soma os contratos de todas as colunas de cada ano para gerar o total
