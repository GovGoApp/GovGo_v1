...
OUT_PATH = PATH + "ITENS\\"
IN_FILE = "CONTRATAÇÕES_PNCP_2025_05.xlsx"
OUT_FILE = "ITENS_CONTRATAÇÕES_PNCP_2025_05.xlsx"
...
# Função para processar um batch específico
def process_batch(sheet_name, batch_df, batch_info, progress):
    start_idx, end_idx, batch_num = batch_info
    batch_size = len(batch_df)
    
    # Adicionar barra para o batch atual (thread-safe)
    with progress_lock:
        batch_id = progress.add_task(
            f"[bold yellow]  {sheet_name} - Batch {batch_num} ({start_idx+1}-{end_idx})", 
            total=batch_size
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            try:
                ...
                with progress_lock:
                    progress.console.log(f"[red]Erro inesperado na thread: {str(e)}")
            
            # Atualizar a barra de progresso do batch
            processados += 1
            with progress_lock:
                progress.update(batch_id, completed=processados)
    ...
    if resultados:
        ...
        num_resultados = len(batch_result_df)
        
        # Atualizar descrição do batch para mostrar conclusão
        with progress_lock:
            progress.update(batch_id, 
                description=f"[bold green]  {sheet_name} - Batch {batch_num:02d} ✓ {num_resultados} itens")
        
        return num_resultados
    else:
        with progress_lock:
            progress.remove_task(batch_id)
            progress.console.log(f"[yellow]⚠ {sheet_name} - Batch {batch_num}: Sem resultados")
        return 0
...
def process_sheet(sheet_name, df, progress):
    ...
    for start_idx in range(0, total_rows, BATCH_SIZE):
        ...
        batch_info = (start_idx, end_idx, start_idx//BATCH_SIZE + 1)
        batches.append((sheet_name, batch_df, batch_info))
    
    resultados_totais = 0
    
    # Processar os batches em paralelo
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(process_batch, b[0], b[1], b[2], progress) for b in batches]
        for future in concurrent.futures.as_completed(futures):
            try:
                num_resultados = future.result()
                resultados_totais += num_resultados
            except Exception as e:
                with progress_lock:
                    progress.console.log(f"[bold red]Erro no processamento de batch: {str(e)}")
...
def main():
    ...
    with progress:
        # Task externa: processamento global
        outer_task = progress.add_task(f"[bold green]📊 Processando Itens de Contratações do dia {today.strftime('%d/%m/%Y')}", total=len(sheets))
        
        progress.console.log(f"[bold cyan]🚀 Iniciando processamento com {MAX_WORKERS} workers em paralelo")
        
        total_itens = 0
        
        # Processar uma aba de cada vez (as abas são sequenciais, mas os batches são paralelos)
        for i, sheet in enumerate(sheets, 1):
            try:
                progress.console.log(f"[bold magenta]📋 Aba {i}/{len(sheets)}: {sheet}")
                df_sheet = pd.read_excel(xls, sheet_name=sheet)
                num_itens = process_sheet(sheet, df_sheet, progress)
        ...
        progress.console.log(f"[bold green]🏁 Processamento concluído! Total de {total_itens} itens processados.")
    
    print(f"\n📄 Planilha de itens do dia {today.strftime('%d/%m/%Y')} salva em: {OUT_PATH + OUT_FILE}")
