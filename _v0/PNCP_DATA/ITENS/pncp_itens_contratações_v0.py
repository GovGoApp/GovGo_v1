...
def process_row(row):
    # Extrai os dados necessários; se a coluna "numeroControlePNCP" não existir, usa ""
    ...
    if not (cnpj and anoCompra and sequencialCompra):
        # Se faltar algum dado, retorna lista vazia
        return []
    # Monta a URL (note o formato com double slash após api/pncp)
    url = f"https://pncp.gov.br/api/pncp//v1/orgaos/{cnpj}/compras/{anoCompra}/{sequencialCompra}/itens"
    try:
        response = requests.get(url)
        if response.status_code != 200:
            return []
        itens = response.json()  # espera-se uma lista
        resultados = []
        for item in itens:
            # Cria um dicionário com os 4 dados da contratação
            registro = {
            ...
            # Acrescenta os campos do item, conforme a ordem definida em ITEM_FIELDS
...
def process_sheet(sheet_name, df, progress):
    resultados = []
    total_rows = len(df)
    # Cria uma task para processar as contratações desta aba (em amarelo, indentado)
    task_id = progress.add_task(f"[yellow]  {sheet_name}", total=total_rows)
    ...
    # Define a ordem das colunas de saída: os 4 dados primeiro e depois os campos dos itens
    ...
    if resultados:
        ...
        # Se houver colunas ausentes, garante que todas existam
...
def main():
    ...
    OUT_PATH = PATH + "ITENS\\"
    IN_FILE = "CONTRATAÇÕES_PNCP_2022.xlsx"
    OUT_FILE = "ITENS_CONTRATAÇÕES_PNCP_2022.xlsx"
    
    # Lê o arquivo Excel com todas as abas
    xls = pd.ExcelFile(PATH + IN_FILE)
    sheets = xls.sheet_names  # lista de nomes de abas
    
    resultados_sheets = {}  # dicionário para armazenar o DataFrame de cada aba
    
    # Cria uma instância global de Progress para exibir tasks
    progress = Progress(
    ...
    with progress:
        # Task externa: Processamento das abas
    ...
    # Ordena as abas na mesma ordem do IN_FILE
    ordered_sheets = sheets
    
    # Salva as abas de saída em um único arquivo Excel
