...
OUTPUT_PATH = os.path.join(CNAE_PATH, "OUTPUT_CNAE_CAT\\")
...
def get_top10_cats(cnae_embedding, cat_embeddings):
    """Calcula a similaridade e retorna os top 10 CATs mais similares para cada CNAE."""
    # Calcular similaridade via cosseno
    sims = cosine_similarity([cnae_embedding], cat_embeddings)[0]
...
def process_combination(cat_sheet, cnae_sheet):
    """Processa uma combinação específica de CNAE → CAT."""
    console.print(Panel.fit(f"[bold green]Processando combinação: {cnae_sheet} → {cat_sheet}[/bold green]"))
    ...
    # 4. Converter embeddings de CAT para matriz
    cat_embeddings_matrix = np.vstack(cat_df['embedding'].to_list())
    ...
    with Progress(
        ...
        sim_task = progress.add_task("[cyan]Calculando similaridades...", total=cnae_df.shape[0])
        
        for index, row in cnae_df.iterrows():
            cnae_id = row['CODIGO'] if 'CODIGO' in cnae_df.columns else index
            cnae_desc = row['NOME']
            cnae_embed = row['embedding']
            
            # Obter os top 10 CATs mais similares
            top10_idx, top10_scores = get_top10_cats(cnae_embed, cat_embeddings_matrix)
            
            # Criar um registro para esta CNAE
            cnae_record = {
                'cnae_id': cnae_id,
            ...
            for i, idx in enumerate(top10_idx, 1):
                cat_id = cat_df.iloc[idx]['CODCAT']
                cat_desc = cat_df.iloc[idx]['NOMCAT']
                
                # Combinar código e nome no formato solicitado
                cnae_record[f'top_{i}'] = f"{cat_id} - {cat_desc}"
            
            # Depois adicionar todos os SCOREs
            for i, score in enumerate(top10_scores, 1):
                cnae_record[f'score_{i}'] = score
            
            # Adicionar à lista de resultados
            result_data.append(cnae_record)
    ...
    # 7. Salvar resultados - nome de arquivo ajustado para refletir o novo sentido
    output_file = os.path.join(OUTPUT_PATH, f'{cnae_sheet}_to_{cat_sheet}_OPENAI_{TIMESTAMP}.xlsx')
...
##########################################
def main():
    console.print(Panel.fit("[bold yellow]CNAE-CAT Mapeamento v1 - Múltiplas Combinações (Inverso)[/bold yellow]"))
    ...
    for cnae_sheet in CNAE_SHEETS:
        for cat_sheet in CAT_SHEETS:
            try:
                ...
                processed_files.append((cnae_sheet, cat_sheet, output_file))
            except Exception as e:
                console.print(f"[bold red]Erro ao processar {cnae_sheet} → {cat_sheet}: {str(e)}[/bold red]")
    ...
    summary_table.add_column("CNAE", style="cyan")
    summary_table.add_column("CAT", style="green")
    summary_table.add_column("Arquivo de Saída", style="yellow")
    
    for cnae_sheet, cat_sheet, output_file in processed_files:
        summary_table.add_row(cnae_sheet, cat_sheet, os.path.basename(output_file))
