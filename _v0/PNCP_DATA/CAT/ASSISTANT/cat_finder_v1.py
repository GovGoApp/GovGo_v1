...
OUT_FILE = os.path.join(REPORTS_PATH, "TESTE_OUT_v1.xlsx")
...
def process_excel(file_path: str, output_path: str):
    """
    Lê a planilha, processa a coluna 'objetoCompra' utilizando 20 threads concorrentes,
    ...
    processed_count = 0
    futures = []
    # Cria um executor com 20 threads
    with ThreadPoolExecutor(max_workers=20) as executor, Progress(
        SpinnerColumn(),
        BarColumn(),
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(objeto):
                ...
                futures.append(executor.submit(process_row, idx, objeto))
        ...
        for future in as_completed(futures):
            ...
            console.print(f"[bold cyan]Linha {idx+1} - Input:[/bold cyan] {df.at[idx, 'objetoCompra']}")
            console.print(f"[bold magenta]Linha {idx+1} - Output:[/bold magenta] {cat} ; {grupo}")
