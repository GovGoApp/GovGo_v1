### EMB_NV2_v5 (EMBEDDING NIVEL 2 V5) ###
...
# Modificação: Substituída a API OpenAI pela biblioteca sentence-transformers para geração de embeddings localmente,
# eliminando custos e dependência de API externa.
...
# Carga do modelo de embeddings (local)
#MODEL_NAME = "paraphrase-multilingual-MiniLM-L12-v2"  # 384 dimensões, Modelo multilíngue que funciona bem com português
MODEL_NAME = "paraphrase-multilingual-mpnet-base-v2"  # 768 dimensões, melhor qualidade
...
def get_model():
    ...
    with model_lock:
        if embedding_model is None:
            console.print(f"[bold magenta]Carregando modelo de embeddings: {MODEL_NAME}...[/bold magenta]")
    ...
    if show_progress:
        with Progress(
            ...
            for i in range(0, len(texts), batch_size):
                ...
                # Obter embeddings usando sentence-transformers
...
def classify_items_batched(df_items, catmat_embeddings, catmat_meta, catser_embeddings, catser_meta, 
    ...
    try:
        ...
        # Não remover o CHECKPOINT_FILE mais    
        #if os.path.exists(CHECKPOINT_FILE):
...
def main():
    ...
    try:
        ...
        console.print(f"[bold blue]Modelo de embeddings utilizado: {MODEL_NAME}[/bold blue]")
