...
def classify_items_batched(df_items, catmat_embeddings, catmat_meta, catser_embeddings, catser_meta, 
    ...
    try:
        # Gerar nomes das colunas TOP e SCORE
        top_columns = [f"TOP_{i+1}" for i in range(top_n)]
        score_columns = [f"SCORE_{i+1}" for i in range(top_n)]
        
        # Definir a ordem das colunas
        base_columns = ["id", "objetoCompra", "tipoDetectado", "categoriaMelhor", "scoreCategoria"]
        ordered_columns = base_columns + top_columns + score_columns
        
        # Adicionar quaisquer outras colunas que possam existir, exceto categoriasTopN (agora redundante)
        other_columns = [col for col in all_results.columns if col not in ordered_columns and col != "categoriasTopN"]
        ordered_columns.extend(other_columns)
        
        # Reordenar dataframe com as colunas na ordem desejada (somente as colunas que existem)
        existing_ordered_columns = [col for col in ordered_columns if col in all_results.columns]
        all_results = all_results[existing_ordered_columns]
        
    except Exception as e:
        console.print(f"[bold red]Erro ao reordenar colunas: {str(e)}[/bold red]")
