...
MAX_WORKERS = 100  # Número de threads para processamento paralelo
...
def classify_items_batched(df_items, catmat_embeddings, catmat_meta, catser_embeddings, catser_meta, 
    ...
    """Classificar itens em batches com processamento paralelo."""
    ...
    for batch_start in range(0, total_items, BATCH_SIZE):
        ...
        # Gerar argumentos para o ThreadPoolExecutor
        process_args = []
        for idx, row in df_batch.iterrows():
            item_id = row.get("id") or row.get("id_pncp")
            description = str(row.get("objetoCompra") or "")
            process_args.append((idx, item_id, description, catmat_matrix, catser_matrix, 
                               catmat_meta, catser_meta, top_n))
        
        # Processar itens em paralelo
        with Progress(
            SpinnerColumn(),
            TextColumn("[bold cyan]Classificando batch..."),
            BarColumn(),
            TaskProgressColumn(),
            TimeElapsedColumn()
        ) as progress:
            task_items = progress.add_task("", total=len(df_batch))
            
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = [executor.submit(process_item, args) for args in process_args]
