...
def run_embedding_pipeline(provider_type, model_name, excel_file=EXCEL_FILE, sheet=SHEET, 
                         catmat_file=CATMAT_FILE, catser_file=CATSER_FILE, top_n=TOP_N):
    """Executa o pipeline completo de classificação para um determinado provedor e modelo"""
    ...
    try:
        ...
        if provider_type.lower() == 'openai':
            # Tentar carregar API key de variável de ambiente
            api_key = os.environ.get("OPENAI_API_KEY")
            if not api_key:
                console.print("[bold red]API key da OpenAI não encontrada! Configure a variável de ambiente OPENAI_API_KEY[/bold red]")
                return None
            provider_kwargs['api_key'] = api_key
        
        console.print(f"\n[bold blue]===== Processando provedor: {provider_type} - modelo: {model_name} =====[/bold blue]")
        provider = get_embedding_provider(provider_type, model_name, **provider_kwargs)
        
        # Carregar dados
        df_items, catmat, catser, existing_results, checkpoint = load_data(
            excel_file=excel_file,
            sheet=sheet,
            catmat_file=catmat_file,
            catser_file=catser_file
        )
        last_processed = checkpoint['last_processed'] if checkpoint else 0
        
        # Preparar textos de catálogo
        catmat_texts, catmat_meta, catser_texts, catser_meta = prepare_catalog_entries(catmat, catser)
        
        # Definir caminhos para embeddings
        ...
        # Verificar/gerar embeddings de catálogo
        ...
        if catmat_embeddings is None or len(catmat_embeddings) != len(catmat_texts):
            console.print(f"[yellow]Gerando embeddings CATMAT com {model_name}...[/yellow]")
        ...
        if catser_embeddings is None or len(catser_embeddings) != len(catser_texts):
            console.print(f"[yellow]Gerando embeddings CATSER com {model_name}...[/yellow]")
            catser_embeddings = provider.embed(catser_texts)
            save_embeddings(catser_embeddings, catser_embed_file)
        
        # Normalizar vetores
        console.print("[cyan]Normalizando vetores...[/cyan]")
        ...
        # Classificar itens
        console.print("[bold magenta]Iniciando classificação...[/bold magenta]")
        ...
        console.print(f"[green]Velocidade: {items_per_second:.1f} itens/segundo[/green]")
        ...
        console.print(f"[bold red]Falha no pipeline para {provider_type}/{model_name}: {str(e)}[/bold red]")
...
def consolidate_results(all_pipeline_results, output_file):
    """Consolida resultados de múltiplos modelos em uma única planilha comparativa"""
    if not all_pipeline_results:
        console.print("[bold red]Nenhum resultado para consolidar![/bold red]")
        return None
    
    console.print("[bold magenta]Consolidando resultados dos modelos...[/bold magenta]")
    
    # Inicializar com o primeiro DataFrame para obter colunas comuns
    base_df = None
    for result in all_pipeline_results:
        if result is not None and 'results' in result:
            base_df = result['results']
            break
    
    if base_df is None:
        console.print("[bold red]Nenhum resultado válido para consolidar![/bold red]")
        return None
    
    # Criar DataFrame consolidado com colunas essenciais
    consolidated_df = base_df[['id', 'objetoCompra', 'tipoDetectado']].copy()
    ...
    for result in all_pipeline_results:
        ...
        column_name = f"{provider_name} - {model_name}"
        
        # Adicionar a melhor categoria como nova coluna
        consolidated_df[column_name] = result['results']['categoriaMelhor']
        consolidated_df[f"{column_name}_SCORE"] = result['results']['scoreCategoria']
        
        # Adicionar TOP_1 para comparação
        if 'TOP_1' in result['results'].columns:
            consolidated_df[f"{column_name}_TOP_1"] = result['results']['TOP_1']
    
    # Adicionar metadados dos modelos como nova aba
    models_metadata = []
    for result in all_pipeline_results:
        if result is None:
            continue
        models_metadata.append({
            ...
            'Itens por segundo': result['items_per_second']
        })
    
    metadata_df = pd.DataFrame(models_metadata)
    
    # Salvar resultados consolidados
    with pd.ExcelWriter(output_file) as writer:
        consolidated_df.to_excel(writer, sheet_name='Resultados Consolidados', index=False)
        metadata_df.to_excel(writer, sheet_name='Metadados dos Modelos', index=False)
    
    console.print(f"[bold green]Resultados consolidados salvos em: {output_file}[/bold green]")
    return consolidated_df

def main():
    """Função principal para executar testes comparativos entre diferentes modelos"""
    console.print("[bold blue]===== SISTEMA UNIFICADO DE COMPARAÇÃO DE MODELOS DE EMBEDDING =====")
    ...
    models_to_test = [
        ...
        # LM Studio (só executar se o LMStudio estiver rodando)
        ('lmstudio', 'text-embedding-nomic-embed-text-v1.5'),
        ('lmstudio', 'text-embedding-granite-embedding-107m-multilingual'),
        ('lmstudio', 'text-embedding-granite-embedding-278m-multilingual'),
        
        # Ollama (só executar se o Ollama estiver instalado)
        ('ollama', 'mxbai-embed-large'),
        ('ollama', 'bge-m3'),
        
        # OpenAI (só executar se tiver API key)
        # ('openai', 'text-embedding-3-small'),
        # ('openai', 'text-embedding-3-large'),
        
        # HuggingFace (mais pesados, exigem mais recursos)
    ...
    # Usar arquivo de teste simplificado para comparação rápida
    excel_file = os.path.join(CLASS_PATH, "TESTE_SIMPLES.xlsx")
    
    # Timestamp para o arquivo consolidado
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    consolidated_output = os.path.join(CLASS_PATH, f"COMPARATIVO_MODELOS_{timestamp}.xlsx")
    
    all_results = []
    
    # Executar cada modelo
    for provider_type, model_name in models_to_test:
        try:
            ...
            if result:
                ...
                console.print(f"[green]✓ Concluído: {provider_type} - {model_name}[/green]")
            else:
                console.print(f"[yellow]✗ Falhou: {provider_type} - {model_name}[/yellow]")
                
        except Exception as e:
            console.print(f"[bold red]Erro ao processar {provider_type} - {model_name}: {str(e)}[/bold red]")
            console.print(traceback.format_exc())
    
    # Consolidar resultados
    if all_results:
        consolidated_df = consolidate_results(all_results, consolidated_output)
        
        # Executar o score_answer_v3 para avaliar os resultados se disponível
        try:
            score_script = os.path.join(BASE_PATH, "CLASSIFICAÇÃO", "COMPARATIVOS", "score_answer_v3.py")
            if os.path.exists(score_script):
                console.print("[bold magenta]Executando avaliação dos resultados...[/bold magenta]")
                subprocess.run(['python', score_script, '--input', consolidated_output])
                console.print("[bold green]Avaliação dos modelos concluída![/bold green]")
        except Exception as e:
            console.print(f"[bold yellow]Não foi possível executar a avaliação: {str(e)}[/bold yellow]")
            
        console.print(f"[bold green]Processamento completo! Resultados consolidados em: {consolidated_output}[/bold green]")
    else:
        console.print("[bold red]Nenhum modelo foi processado com sucesso.[/bold red]")
