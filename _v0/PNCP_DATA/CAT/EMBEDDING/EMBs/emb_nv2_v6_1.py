### EMB_NV2_v6 (EMBEDDING NIVEL 2 V6) ###
...
embedding_model = None  # Será carregado sob demanda para economizar memória inicial
...
LMMODEL_PATH = PATH + ".lmstudio\\models\\" 



# Substituir a definição do modelo
MODEL_TYPE = "gguf"  # Opções: "sentence_transformer" ou "gguf"

# Modelo de Embedding em formato gguf 
GGUF_MODEL_NAME = LMMODEL_PATH + "lmstudio-community\\granite-embedding-278m-multilingual-GGUF\\granite-embedding-278m-multilingual-Q4_K_M.gguf"
console.print(f"[bold blue]Modelo GGUF: {GGUF_MODEL_NAME}[/bold blue]")
#GGUF_MODEL_NAME = LMMODEL_PATH + "gaianet\\Nomic-embed-text-v1.5-Embedding-GGUF\\nomic-embed-text-v1.5.f16.gguf"

# Carga do modelo de Sequence Transformer
#ST_MODEL_NAME = "paraphrase-multilingual-MiniLM-L12-v2"  # 384 dimensões, Modelo multilíngue que funciona bem com português
ST_MODEL_NAME = "paraphrase-multilingual-mpnet-base-v2"  # 768 dimensões, melhor qualidade

# Definir MODEL_NAME com base no tipo escolhido
if MODEL_TYPE == "sentence_transformer":
    MODEL_NAME = ST_MODEL_NAME
elif MODEL_TYPE == "gguf":
    MODEL_NAME = GGUF_MODEL_NAME.split("\\")[-1].replace(".gguf", "")  # Nome simplificado para arquivos
else:
    raise ValueError(f"Tipo de modelo '{MODEL_TYPE}' não reconhecido!")

console.print(f"[bold blue]Tipo de modelo: {MODEL_TYPE}")
...
OUTPUT_FILE = CLASS_PATH + f"CLASSIFICACAO_EMB_LOCAL_{TIMESTAMP}.xlsx"
#OUTPUT_FILE = CLASS_PATH + f"TESTE_SIMPLES_EMB_LOCAL_{TIMESTAMP}.xlsx"
...
# Constantes para controle de execução# Constantes para controle de execução
MAX_WORKERS = max(32, os.cpu_count() * 4) # Número de threads para processamento paralelo
...
model_lock = threading.Lock()  # Para carregamento thread-safe do modelo

# Modificar a função get_model()
#
def get_model():
    """Carrega o modelo de embeddings sob demanda (lazy loading)"""
    global embedding_model, MODEL_TYPE
    with model_lock:
        if embedding_model is None:
            if MODEL_TYPE == "sentence_transformer":
                console.print(f"[bold magenta]Carregando modelo sentence-transformers: {ST_MODEL_NAME}...[/bold magenta]")
                embedding_model = SentenceTransformer(ST_MODEL_NAME)
                console.print(f"[green]Modelo carregado com sucesso![/green]")
            elif MODEL_TYPE == "gguf":
                try:
                    # Converter caminho para Path para resolver problemas com espaços
                    from pathlib import Path
                    model_path = str(Path(GGUF_MODEL_NAME))
                    
                    console.print(f"[bold magenta]Carregando modelo GGUF: {model_path}...[/bold magenta]")
                    
                    # Configuração específica para evitar problemas de acesso à memória
                    embedding_model = Llama(
                        ...
                        n_ctx=512,
                        n_batch=512,
                        n_threads=4,  # Reduzir para evitar problemas de concorrência
                        use_mmap=False,  # Tentar sem memory mapping
                        use_mlock=False  # Não bloquear na memória
                    )
                    console.print(f"[green]Modelo GGUF carregado com sucesso![/green]")
                except Exception as e:
                    console.print(f"[bold red]Erro fatal ao carregar modelo GGUF: {str(e)}[/bold red]")
                    import traceback
                    console.print(traceback.format_exc())
                    
                    # Se você realmente não quer fallback:
                    console.print(f"[bold red]Saindo: não é possível continuar sem o modelo GGUF.[/bold red]")
                    import sys
                    sys.exit(1)  # Encerrar o programa se o modelo GGUF não carregar
...
def get_embeddings(texts, batch_size=32, show_progress=True):
    """Gerar embeddings para uma lista de textos usando sentence-transformers localmente."""
    ...
    if show_progress:
        with Progress(
            ...
            for i in range(0, len(texts), batch_size):
                ...
                # Obter embeddings usando o modelo configurado
                if MODEL_TYPE == "sentence_transformer":
                    ...
                    batch_embeddings = []
                    for text in batch:
                        # O GGUF/llama.cpp tem uma API diferente
...
def classify_items_batched(df_items, catmat_embeddings, catmat_meta, catser_embeddings, catser_meta, 
    ...
    for batch_start in range(0, total_items, BATCH_SIZE):
        ...
        # 2. Obter embeddings para todo o batch de uma vez (operação local agora)
        ...
        try:
            ...
            if valid_descriptions:
                # Obter embeddings usando nosso modelo local
                # O tamanho do batch é maior para embeddings locais (mais eficiente)
                item_embeddings = get_embeddings(valid_descriptions, batch_size=64, show_progress=True)
    ...
    try:
        ...
        CHECK_TIMESTAMP = datetime.now().strftime('%Y%m%d_%H%M')
...
def main():
    ...
    try:
        ...
        console.print(f"[bold blue]Tipo de modelo utilizado: {MODEL_TYPE}")
        console.print(f"[bold blue]Modelo de embeddings utilizado: {MODEL_NAME}")
