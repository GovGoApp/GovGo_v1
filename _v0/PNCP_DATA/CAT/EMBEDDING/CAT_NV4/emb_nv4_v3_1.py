### EMB_NV4_v3_1 (EMBEDDING NIVEL 4 V3.1) ###

# Este script é responsável por gerar embeddings para itens de compras e classificá-los em categorias unificadas.
# Modificação: Paralelismo DENTRO de cada arquivo, processando arquivos sequencialmente.
import os
import re
import sys
import json
...
# Gerenciamento de posições para tqdm
...
def get_embeddings(texts, model=EMBEDDING_MODEL, batch_size=100, show_progress=True, position=0, worker_id=None):
    ...
    if show_progress:
        ...
        for i in tqdm(range(0, len(texts), batch_size), 
                     total=total_batches, 
                     desc=f"{worker_prefix} - Embeddings", 
                     position=position,
                     leave=True,
...
def process_chunk(chunk_df, cat_embeds, cat_meta, worker_id, position):
    """
    Processa um chunk específico do DataFrame com sua própria barra de progresso.
    Posições organizadas e estáticas para cada worker.
    """
    ...
    # Calcular posições para as 3 barras de progresso deste worker
    # Cada worker ocupa 3 linhas consecutivas no terminal
    lines_per_worker = 3
    base_position = position * lines_per_worker
    
    # Posição fixa para as 3 barras deste worker
    preproc_position = base_position
    embed_position = base_position + 1
    sim_position = base_position + 2
    ...
    # Pré-processar os textos com barra de progresso
    item_texts = []
    
    # Barra 1: Pré-processamento (posição fixa)
    for text in tqdm(raw_texts, 
                    desc=f"{worker_prefix} - Pré-processamento",
                    position=preproc_position,
                    leave=True,
                    bar_format='{l_bar}{bar:30}{r_bar}'):
        item_texts.append(preprocess_text(text))
    
    # Barra 2: Embeddings (posição fixa, logo abaixo da barra 1)
    ...
    # Barra 3: Similaridade (posição fixa, logo abaixo da barra 2)
    for idx, item_embed in tqdm(enumerate(item_embeds), 
                              total=len(item_embeds),
                              desc=f"{worker_prefix} - Similaridade",
...
def process_file_parallel(input_file, cat_embeddings, cat_meta, file_idx):
    ...
    e processa cada chunk em uma thread separada.
    """
    try:
        ...
        # Limpar a tela completamente antes de iniciar novo arquivo
        os.system('cls' if os.name == 'nt' else 'clear')
        
        # Define quantas linhas cada worker ocupa na tela (3 tqdms por worker)
        lines_per_worker = 3
        
        # Calcular número total de linhas necessárias para as barras de progresso
        num_workers = min(MAX_WORKERS, total_chunks)
        total_lines_needed = (lines_per_worker * num_workers) + 5
        
        # Reservar espaço no terminal
        print("\n" * total_lines_needed)
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for chunk_idx in range(min(total_chunks, MAX_WORKERS)):
                ...
                # Posição para este worker (0, 1, 2, ...)
                position = chunk_idx
                ...
                future = executor.submit(
                    ...
                    cat_embeddings, 
                    cat_meta, 
                    chunk_idx + 1,  # worker_id (1-based)
                    position        # position (0-based)
                )
                futures.append((future, chunk_idx))
            
            # Coletar resultados usando uma posição fixa para o tqdm global
            master_position = total_lines_needed + 1
            
            # Processar resultados à medida que ficam prontos
            for future, chunk_idx in tqdm(futures, 
                                         desc="Progresso dos chunks", 
            ...
            for chunk_idx in range(MAX_WORKERS, total_chunks):
                ...
                # Usar posição reciclada
                position = chunk_idx % MAX_WORKERS
                
                try:
                    # Processar chunk diretamente
                    result = process_chunk(
                        chunk_df, 
                        cat_embeddings, 
                        cat_meta, 
                        position + 1,  # worker_id (1-based)
                        position       # position (0-based)
                    )
                    chunks_results.append(result)
                except Exception as e:
...
def main():
    ...
    try:
        ...
        with tqdm(total=len(remaining_files), desc="Progresso de arquivos", position=MAX_WORKERS * 3 + 5) as progress_bar:
            for idx, input_file in enumerate(remaining_files, 1):
                ...
                # Pausa visual entre arquivos, mas não limpa a tela aqui
                time.sleep(1)
