...
# Instância do cliente OpenAI (atenção à API key)
...
global_similarity_lock = threading.Lock()  # Para a barra de progresso global
...
def prepare_catalog_entries(cat):
    """Prepara as entradas do catálogo para gerar embeddings."""
...
def process_text_batch(texts):
    """Aplica o pré-processamento a um lote de textos."""
    return [preprocess_text(text) for text in texts]

def process_complete_batch(batch_df, cat_embeds, cat_meta, batch_id, start_idx, log_queue, batch_status, global_progress, global_task):
    """Processa um lote completo com logs detalhados e atualiza a barra de progresso global de similaridades."""
    ...
    try:
        ...
        # Fase 4: Cálculo de similaridades com atualização da barra de progresso global
        total_items_batch = len(item_embeds)
        ...
        for idx, item_embed in enumerate(item_embeds):
            ...
            # Atualiza a barra de progresso global a cada 100 itens ou no final
            if processed_in_batch % 100 == 0 or idx == total_items_batch - 1:
                with global_similarity_lock:
                    global_progress.update(global_task, advance=processed_in_batch)
                processed_in_batch = 0
            if idx % 1000 == 0 or idx == total_items_batch - 1:
                perc = int(100 * (idx + 1) / total_items_batch)
                log_queue.put((batch_id, f"Lote {batch_id} - Calculando similaridades: {perc}% concluído.", "Cálculo"))
...
def main():
    ...
    # Cria o layout com duas áreas: status dos batches e barra de progresso global
    layout = Layout()
    layout.split_column(
        Layout(name="status"),
        Layout(name="global", size=4)
    )
    # Cria a barra de progresso global para similaridades (total = total itens do Excel)
    overall_progress = Progress(SpinnerColumn(), TextColumn("[bold cyan]{task.description}"), BarColumn(), TimeElapsedColumn())
    overall_task = overall_progress.add_task("Similaridade Total", total=total_items)
    layout["global"].update(overall_progress)
        ...
        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_BATCHES) as executor:
            ...
            for idx, (start_idx, batch_df) in enumerate(batches):
                ...
                if batch_id in processed_batch_ids:
                    continue
                future = executor.submit(process_complete_batch, batch_df, cat_embeddings, cat_meta, batch_id, start_idx, log_queue, batch_status, overall_progress, overall_task)
            ...
            while future_to_batch:
                for future in as_completed(list(future_to_batch.keys())):
                    ...
                    layout["status"].update(create_batch_status_table(batch_status, completed_batches, total_batches))
