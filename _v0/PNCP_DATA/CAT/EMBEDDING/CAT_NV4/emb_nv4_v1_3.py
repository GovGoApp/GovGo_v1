    ...
    for i in range(0, len(texts), batch_size):
        ...
        if (i // batch_size) % 5 == 0 or (i + batch_size >= len(texts)):
            ...
            if batch_id is not None:
                ...
                update_batch_status(
                    batch_id,
                    f"Gerando embeddings: {progress_pct}%",
...
def calculate_direct_similarities(item_embeds, cat_embeds, cat_meta, top_n=10, batch_id=None, log_queue=None):
    """Calcula as TOP_N categorias mais similares diretamente para cada item."""
    num_items = len(item_embeds)
    num_cats = len(cat_embeds)
    
    # Reportar início
    start_time = time.time()
    if log_queue and batch_id:
        log_queue.put((batch_id, f"Iniciando cálculo direto de similaridades para {num_items} itens", "Similaridade"))
    ...
    # Normalizar embeddings de categorias (apenas uma vez)
    cat_embeds_array = np.vstack(cat_embeds)
    cat_norms = np.linalg.norm(cat_embeds_array, axis=1)
    cat_norms = np.maximum(cat_norms, 1e-10)  # Evitar divisão por zero
    normalized_cats = cat_embeds_array / cat_norms[:, np.newaxis]
    
    # Processar cada item
    for i in range(num_items):
        # Normalizar embedding do item atual
        item_embed = item_embeds[i]
        item_norm = np.linalg.norm(item_embed)
        item_norm = max(item_norm, 1e-10)  # Evitar divisão por zero
        normalized_item = item_embed / item_norm
        
        # Calcular similaridades para todas as categorias
        similarities = np.zeros(num_cats)
        for j in range(num_cats):
            similarities[j] = np.dot(normalized_item, normalized_cats[j])
        
        # Encontrar os TOP_N maiores índices
        indices = np.argpartition(similarities, -top_n)[-top_n:]
        # Ordenar apenas os TOP_N índices
        sorted_local_indices = np.argsort(-similarities[indices])
        # Reordenar os índices originais
        sorted_indices = indices[sorted_local_indices]
        
        top_indices[i] = sorted_indices
        top_scores[i] = similarities[sorted_indices]
        
        # Reportar progresso periodicamente
        if i % report_interval == 0 or i == num_items - 1:
            progress_pct = min(100, int(100 * (i + 1) / num_items))
            elapsed = time.time() - start_time
            rate = (i+1) / elapsed if elapsed > 0 else 0
            eta = (num_items - i - 1) / rate if rate > 0 else 0
            
            # Atualizar fila de logs
            if log_queue and batch_id:
                log_queue.put((
                    batch_id, 
                    f"Similaridade: {progress_pct}% ({i+1}/{num_items}) • {rate:.1f} itens/s • ETA: {eta:.1f}s", 
                    "Similaridade"
                ))
            
            # Atualizar a barra de progresso - mapear 0-100% para 60-85% do progresso total
            if batch_id is not None:
    ...
    if log_queue and batch_id:
        ...
        log_queue.put((
            batch_id, 
            f"✅ Similaridades diretas calculadas em {total_time:.2f}s ({num_items/total_time:.1f} itens/s)", 
            "Similaridade"
        ))
