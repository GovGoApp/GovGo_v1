#!/usr/bin/env python
# -*- coding: utf-8 -*-
### EMB_NV4_v1_3 - Versão com processamento de lotes em paralelo confiável
### VERSÃO v1.3: Similaridade direta (sem matriz)
### Calculo direto item-a-item
...
MAX_CONCURRENT_BATCHES = os.cpu_count() * 2  # Máximo de lotes ativos simultaneamente
...
def get_embeddings(texts, model=EMBEDDING_MODEL, batch_size=25, batch_id=None):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        # Log periódico e atualizar barra de progresso
        if (i // batch_size) % 5 == 0 or (i + batch_size >= len(texts)):
            ...
            # Atualizar a barra de progresso - mapear 0-100% para 35-50% do progresso total
            if batch_id is not None:
                batch_progress = 35 + int((progress_pct / 100) * 15)
                update_batch_status(
                    batch_id,
                    f"(4) Gerando embeddings: {progress_pct}%",
...
def calculate_direct_similarities(item_embeds, cat_embeds, cat_meta, top_n=10, batch_id=None, log_queue=None):
    """Calcula as TOP_N categorias mais similares diretamente para cada item de forma vetorizada."""
    num_items = len(item_embeds)
    
    # Reportar início
    start_time = time.time()
    if log_queue and batch_id:
        log_queue.put((batch_id, f"Iniciando cálculo vetorizado de similaridades para {num_items} itens", "Similaridade"))
    ...
    report_interval = max(1, num_items // 10)
    
    # Converte listas de embeddings para arrays numpy
    item_embeds_array = np.vstack(item_embeds)
    cat_embeds_array = np.vstack(cat_embeds)
    
    if log_queue and batch_id:
        log_queue.put((batch_id, f"Arrays convertidos, processando similaridades...", "Similaridade"))
    
    # Processa itens em lotes para mostrar progresso
    for i in range(0, num_items, report_interval):
        # Processa um lote de itens
        end_idx = min(i + report_interval, num_items)
        batch_items = item_embeds_array[i:end_idx]
        
        # Para cada item no lote, calcular similaridade com todas as categorias
        for j, item_embed in enumerate(batch_items):
            # Calcular similaridade de forma vetorizada usando o mesmo método de emb_nv2_v4_2.py
            sims = np.array([
                cosine_similarity(item_embed, cat_embed) 
                for cat_embed in cat_embeds
            ])
            
            # Obter índices dos TOP_N mais similares (mais eficiente que argsort completo)
            indices = np.argpartition(sims, -top_n)[-top_n:]
            # Ordenar apenas os TOP_N índices
            sorted_local_indices = np.argsort(-sims[indices])
            # Reordenar os índices originais
            sorted_indices = indices[sorted_local_indices]
            
            # Armazenar resultados
            idx = i + j
            top_indices[idx] = sorted_indices
            top_scores[idx] = sims[sorted_indices]
            
        # Reportar progresso a cada lote
        progress_pct = min(100, int(100 * end_idx / num_items))
        elapsed = time.time() - start_time
        rate = end_idx / elapsed if elapsed > 0 else 0
        eta = (num_items - end_idx) / rate if rate > 0 else 0
        
        # Atualizar logs e barra de progresso
        if log_queue and batch_id:
            log_queue.put((
                batch_id, 
                f"Similaridade: {progress_pct}% ({end_idx}/{num_items}) • {rate:.1f} itens/s • ETA: {eta:.1f}s", 
                "Similaridade"
            ))
        
        batch_progress = 60 + int((progress_pct / 100) * 25)
        update_batch_status(
            batch_id,
            f"Calculando similaridades: {progress_pct}%",
            progress=batch_progress
        )
    ...
    if log_queue and batch_id:
        ...
        log_queue.put((
            batch_id, 
            f"✅ Similaridades calculadas em {total_time:.2f}s ({num_items/total_time:.1f} itens/s)", 
            "Similaridade"
        ))
...
def process_complete_batch(batch_df, cat_embeds, cat_meta, batch_id, start_idx, log_queue):
    ...
    try:
        ...
        update_batch_status(batch_id, "(1) ▶ Iniciando processamento", progress=0)
        ...
        update_batch_status(batch_id, "(2) Extraindo textos...", progress=5)
        raw_texts = []
        ...
        update_batch_status(batch_id, "(3) Pré-processando textos...", progress=20)
        ...
        update_batch_status(batch_id, "(4) Gerando embeddings...", progress=35)
        ...
        # Fase 4: Cálculo direto de similaridades (sem matriz)
        log_queue.put((batch_id, "Calculando similaridades diretamente...", "Similaridade"))
        update_batch_status(batch_id, "(5) Calculando similaridades diretas", progress=60)
        
        # Calcular TOP_N categorias mais similares para cada item diretamente
        top_indices, top_scores = calculate_direct_similarities(embeddings, cat_embeds, cat_meta, TOP_N, batch_id, log_queue)
        
        # Atualizar o DataFrame com os resultados
        update_batch_status(batch_id, "(6) Atualizando resultados...", progress=85)
        ...
        # Salvar resultados
        update_batch_status(batch_id, "(7) Salvando resultados...", progress=95)
...
def main():
    ...
    console.print(Panel.fit("[bold yellow]EMB_NV4_v1_3 - Sistema de classificação com embeddings para grandes volumes[/bold yellow]"))
    console.print("[bold green]Versão com cálculo direto de similaridade (sem matriz) e monitoramento detalhado[/bold green]")
