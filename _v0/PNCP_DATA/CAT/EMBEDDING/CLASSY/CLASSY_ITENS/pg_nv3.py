"""
PG_V1.py - Avaliação de Modelos de Classificação 
-------------------------------------------------
...
RESULTADOS_DIR = BASE_PATH + "ESTUDOS\\P176\\RESULTADOS_NV3\\"
...
def extrair_parametros_do_nome(nome_arquivo):
    ...
    Formato esperado: OUTPUT_ITEM_XXX_HY_DZ_N1_N2_N3_MS_OC_IT.xlsx
    ...
    try:
        ...
        # Padrão: H(\d+)_D(\d+)_(\d+(?:_\d+)*)_(\d)_(\d)_(\d)
...
def processar_output(output_file, samples_df):
    ...
    Agora inclui parâmetros extraídos do nome do arquivo.
    """
    ...
    try:
        ...
        stats = {
            ...
            'acertos_top1_pct': len(resultados_df[resultados_df['posicao_encontrada'] == 1])/len(resultados_df)*100,
            'acertos_top5_pct': len(resultados_df[resultados_df['posicao_encontrada'] > 0])/len(resultados_df)*100,
            'confidence_gt50': len(resultados_df[resultados_df['CONFIDENCE'] > 50])/len(resultados_df)*100,
            'confidence_lt50': len(resultados_df[resultados_df['CONFIDENCE'] <= 50])/len(resultados_df)*100,
            'score1_gt70': len(resultados_df[resultados_df['SCORE_1'] > 0.7])/len(resultados_df)*100,
            'score1_lt70': len(resultados_df[resultados_df['SCORE_1'] <= 0.7])/len(resultados_df)*100
        }
        ...
        console.print(f"  Acertos no TOP 1: {stats['acertos_top1']} ({stats['acertos_top1_pct']:.2f}%)")
        console.print(f"  Acertos no TOP 5: {stats['acertos_top5']} ({stats['acertos_top5_pct']:.2f}%)")
        console.print(f"  CONFIDENCE >50: {stats['confidence_gt50']:.2f}%")
        console.print(f"  SCORE_1 >0.7: {stats['score1_gt70']:.2f}%")
        ...
        if novas_stats or existing_results:
            ...
            if todas_stats:
                ...
                for i, (_, row) in enumerate(ranked_stats.iterrows(), 1):
                    ...
                    pg = row['pg_media']
                    acertos_pct = row['acertos_top1_pct'] if 'acertos_top1_pct' in row and pd.notna(row['acertos_top1_pct']) else 0
