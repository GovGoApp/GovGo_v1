...
INPUT_ITENS_PATH = CLASSY_PATH + "INPUT_ITENS\\"
OUTPUT_ITENS_PATH = CLASSY_PATH + "OUTPUT_ITENS\\"

# Manter os caminhos existentes para embeddings
EMBEDDINGS_PATH = CLASSY_PATH + "EMBEDDING\\"

# Novo caminho para embeddings dos itens
INPUT_ITENS_EMBEDDINGS_PATH = CLASSY_PATH + "INPUT_ITENS_EMBEDDINGS\\"
...
def classify_items_combined(df_items, indices, cat_meta_by_level, cat_embeddings_by_level, embedding_function):
    """Versão modificada para processar arquivos INPUT_ITENS."""
    ...
    # O resto do processamento segue igual
    progress = Progress(
        SpinnerColumn(),
        TextColumn("[bold blue]{task.description}"),
        BarColumn(),
    ...
    # Ordenar colunas para output - adaptado para manter as colunas de INPUT_ITENS
    if OUTPUT_DEBUG:
        desired_columns = ['numeroControlePNCP', 'numeroItem', 'ID_ITEM_CONTRATACAO', 'descrição']
        for i in range(1, TOP_N + 1):
            desired_columns.append(f"TOP_{i}")
        for i in range(1, TOP_N + 1):
            desired_columns.append(f"SCORE_{i}")
        desired_columns.append("CONFIDENCE")
        # Adicionar colunas de debug também
        final_columns = [col for col in result_df.columns]
        return result_df[final_columns]  # Garante retorno mesmo em modo debug
    else:
        # Modo não-debug - incluir todas as colunas originais + resultados
        # Primeiro as colunas de ID do arquivo INPUT_ITENS
        desired_columns = ['numeroControlePNCP', 'numeroItem', 'ID_ITEM_CONTRATACAO', 'descrição']
        
        # Depois as categorias e scores
        for i in range(1, TOP_N + 1):
            desired_columns.append(f"TOP_{i}")
        for i in range(1, TOP_N + 1):
            desired_columns.append(f"SCORE_{i}")
        
        # E coluna de confiança
        desired_columns.append("CONFIDENCE")
