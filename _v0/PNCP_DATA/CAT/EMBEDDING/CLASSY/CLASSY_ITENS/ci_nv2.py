...
NOVA_CAT_SHEET = "CAT_NV2" # NV0, NV1 e NV2
...
# Pesos para níveis hierárquicos - modificado para apenas NV1 e NV2 (soma = 1.0)
WEIGHT_NV1 = 0.3
WEIGHT_NV2 = 0.7
...
def prepare_catalog_entries(cat_df):
    ...
    with Progress(
        ...
        # Processamento nível NV2 (usado como direto)
        processed_codcats = set()  # Para evitar duplicatas
        for _, row in cat_df.iterrows():
            # Extrair apenas até NV2 do código completo
            codnv0 = str(row.get('CODNV0', ''))
            codnv1 = str(row.get('CODNV1', '')).zfill(4)
            codnv2 = str(row.get('CODNV2', '')).zfill(5)
            codcat_nv2 = f"{codnv0}{codnv1}{codnv2}"
            
            nomcat = f"{row.get('NOMNV0', '')} :: {row.get('NOMNV1', '')} :: {row.get('NOMNV2', '')}"
            
            if codcat_nv2 not in processed_codcats:
                processed_codcats.add(codcat_nv2)
                combined_text = preprocess_text(nomcat)
            ...
            progress.update(task, advance=0.5)  # 50% do progresso para NV2
        
        # Processamento nível NV1 - CORRIGIDO
        processed_nv1 = set()
        for _, row in cat_df.iterrows():
            ...
            if codcat_nv1 not in processed_nv1:
                ...
                cat_meta_by_level['NV1'].append({
                    'CODCAT': codcat_nv1,  # Adicionado: CODCAT para NV1
                    'NOMCAT': nomcat_nv1,   # Adicionado: NOMCAT para NV1
                    'CODNV0': codnv0,
                    'CODNV1': codnv1,
                    'NOMNV0': nomnv0,
                    'NOMNV1': nomnv1
            ...
            progress.update(task, advance=0.5)  # 50% do progresso para NV1
...
def calculate_direct_similarity(item_embed, cat_embeddings_nv2, cat_meta_nv2):
    ...
    for idx, (cat_idx, similarity) in enumerate(top_indices):
        ...
        if 'CODCAT' in meta:
            ...
            codnv2 = meta.get('CODNV2', '').zfill(5)
            code = f"{codnv0}{codnv1}{codnv2}"
        ...
        if 'NOMCAT' in meta:
            ...
            nomnv2 = meta.get('NOMNV2', '')
            name = f"{nomnv0}; {nomnv1}; {nomnv2}"
        ...
        results.append({
            ...
            'level': 'NV2',
            'level_scores': {'NV2': float(similarity)}
        })
...
def combine_results(hierarchical_results, direct_results):
    ...
    for result in direct_results:
        ...
        if code in all_candidates:
            ...
            # Atualizar o nível NV2 se não existir
            if 'NV2' not in all_candidates[code]['level_scores']:
                all_candidates[code]['level_scores']['NV2'] = direct_score
        else:
...
def create_hierarchical_indices(cat_texts_by_level, model=EMBEDDING_MODEL):
    """
    Cria embeddings e índices FAISS para os níveis hierárquicos NV1 e NV2 do catálogo.
    Processamento explícito nível por nível para garantir criação de ambos.
    ...
    """
    console.print("[bold magenta]Criando embeddings e índices hierárquicos NV1 e NV2...[/bold magenta]")
    ...
    # --------------------------------------------------------------------
    # ETAPA 2: Processar explicitamente o nível NV2
    # --------------------------------------------------------------------
    console.print("[bold cyan]ETAPA 2: Processando nível hierárquico NV2...[/bold cyan]")
    level = 'NV2'
    ...
    console.print(f"[cyan]Encontrados {len(texts)} textos para processar em {level}[/cyan]")
    ...
    # Gerar embeddings para NV2
    console.print(f"[cyan]Gerando embeddings para {level}...[/cyan]")
    try:
        nv2_embeddings = get_embeddings(texts, max_workers=1)
        
        if not nv2_embeddings or len(nv2_embeddings) == 0:
            console.print(f"[bold red]ERRO CRÍTICO: Nenhum embedding gerado para {level}[/bold red]")
            return {}, {}
        
        console.print(f"[green]Gerados {len(nv2_embeddings)} embeddings para {level}[/green]")
        
        # Salvar embeddings NV2
        console.print(f"[cyan]Salvando embeddings para {level}...[/cyan]")
        if save_embeddings(nv2_embeddings, level_embed_file):
            console.print(f"[green]Embeddings para {level} salvos com sucesso em {level_embed_file}[/green]")
        else:
            console.print(f"[bold red]ERRO: Falha ao salvar embeddings para {level}[/bold red]")
            return {}, {}
        
        # Criar índice FAISS para NV2
        console.print(f"[cyan]Criando índice FAISS para {level}...[/cyan]")
        embeddings_array = np.array(nv2_embeddings, dtype=np.float32)
        index_nv2 = faiss.IndexFlatIP(embeddings_array.shape[1])
        faiss.normalize_L2(embeddings_array)
        index_nv2.add(embeddings_array)
        
        # Salvar índice NV2
        faiss.write_index(index_nv2, level_index_file)
        console.print(f"[green]Índice FAISS para {level} salvo com sucesso em {level_index_file}[/green]")
        
        # Armazenar em memória
        indices[level] = index_nv2
        embeddings_by_level[level] = nv2_embeddings
    ...
    # --------------------------------------------------------------------
    # ETAPA 3: Verificação final dos embeddings e índices
    # --------------------------------------------------------------------
    console.print("[bold cyan]ETAPA 3: Verificação final dos embeddings e índices...[/bold cyan]")
    ...
    console.print("[bold green]Todos os embeddings e índices para NV1 e NV2 criados com sucesso![/bold green]")
...
def fast_hierarchical_beam_search_combined(item_embed, indices, cat_meta_by_level, cat_embeddings_by_level, hierarchical_maps, item_type=None):
    ...
    Modificada para usar apenas NV1 e NV2.
    """
...
def calculate_direct_similarity_with_faiss(item_embed, indices, cat_meta_nv2, item_type=None):
    """
    Versão que usa o índice FAISS existente para busca direta e respeita o tipo (M/S).
    Modificada para usar apenas NV2 como nível direto.
    """
    # Normalizar embedding de consulta
    query_embed = np.array([item_embed], dtype=np.float32)
    faiss.normalize_L2(query_embed)
    ...
    D, I = indices['NV2'].search(query_embed, search_k)
    
    results = []
    for i, (idx, distance) in enumerate(zip(I[0], D[0])):
        meta = cat_meta_nv2[idx]
        
        # Gerar CODCAT_NV2 se não existir
        if 'CODCAT' in meta:
            code = meta['CODCAT']
        else:
...
def process_input_file(input_number):
    ...
    config_suffix = f"{wh_str}_{wd_str}_{nnn_str}_{ms_flag}_{oc_flag}_{it_flag}_NV2"
...
def fast_hierarchical_beam_search(item_embed, indices, cat_meta_by_level, cat_embeddings_by_level, hierarchical_maps, item_type=None):
    ...
    # Lista para coletar TODOS os resultados NV2 antes de ordenação global
    all_nv2_results = []
    
    # Número máximo de elementos em cat_embeddings_by_level['NV2']
    max_nv2_idx = len(cat_embeddings_by_level['NV2']) - 1
    ...
    for idx, (nv1_idx, nv1_similarity) in enumerate(top_nv1):
        ...
        # CORREÇÃO: Filtrar índices inválidos para evitar IndexError
        valid_nv2_indices = [j for j in nv2_indices if 0 <= j <= max_nv2_idx]
        
       
        try:
            # CORREÇÃO: Usar os índices válidos e indexar corretamente o array
            nv2_embeddings = np.array([cat_embeddings_by_level['NV2'][j] for j in valid_nv2_indices])
            nv2_similarities = np.dot(nv2_embeddings, item_embed)
            
            # Criar mapeamento entre índices válidos e originais para manter a referência correta
            valid_to_original_idx = {i: valid_nv2_indices[i] for i in range(len(valid_nv2_indices))}
            
            # Usar os índices locais (0 a len(valid_nv2_indices)-1) em vez dos índices globais
            nv2_with_scores = [(valid_to_original_idx[j], sim, j) for j, sim in enumerate(nv2_similarities)]
            nv2_with_scores.sort(key=lambda x: x[1], reverse=True)
            top_nv2 = nv2_with_scores[:TOP_NV2]
            
            
        except Exception as e:
            console.print(f"[bold red]Erro ao calcular similaridades NV2: {str(e)}[/bold red]")
            import traceback
            console.print(traceback.format_exc())
            continue
        ...
        for rank_nv2, (nv2_idx, nv2_similarity, local_idx) in enumerate(top_nv2):
            # DEBUG: verificar índice antes de acessar
            if nv2_idx < 0 or nv2_idx >= len(cat_meta_by_level['NV2']):
                console.print(f"[bold red]ERRO: Índice NV2 {nv2_idx} fora do intervalo válido (0-{len(cat_meta_by_level['NV2'])-1})[/bold red]")
                continue
            ...
            # Cálculo direto do score ponderado com NV1 e NV2 apenas
            composite_score = WEIGHT_NV1 * nv1_similarity + WEIGHT_NV2 * nv2_similarity
            
            code = nv2_code  # Código NV2 completo
            name = f"{nv2_meta.get('NOMNV0', '')} :: {nv2_meta.get('NOMNV1', '')} :: {nv2_meta.get('NOMNV2', '')}"
    ...
    # Ordenação global dos resultados NV2 para visualização
    if OUTPUT_DEBUG:
...
def precompute_hierarchical_relationships(cat_meta_by_level):
    ...
    console.print(f"[green]Relações hierárquicas pré-calculadas: {len(nv1_to_nv2)} itens NV1->NV2[/green]")
