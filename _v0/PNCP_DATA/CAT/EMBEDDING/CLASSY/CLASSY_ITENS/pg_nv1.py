"""
PG_NV1.py - Avaliação de Modelos de Classificação com Comparação em Nível 1
---------------------------------------------------------------------------
Esta versão implementa uma avaliação simplificada que:
1. Compara apenas os códigos até o nível NV1 (ignorando CODNV2 e CODNV3)
2. Usa um sistema binário de 2 bits (máximo 3) para pontuação
3. Implementa o conceito de CODCAT1 = concatenação de CODNV0 + CODNV1

As métricas foram recalibradas para este modelo de comparação altamente reduzida.
"""
...
RESULTADOS_DIR = BASE_PATH + "ESTUDOS\\P176\\RESULTADOS_NV1\\"  # Novo diretório para resultados NV1
...
# Agora usa 3 (máximo binário 11 = decimal 3) em vez de 7
PSM = 3 * sum(WSS)  # 3 (match perfeito NV1) * soma dos pesos = 3 * 21 = 63
...
def extrair_codigos(codigo_completo):
    ...
    MODIFICAÇÃO: Retorna apenas os primeiros 2 níveis (ignorando CODNV2 e CODNV3)
    
    Args:
        codigo_completo: Código completo no formato especificado
        
    Returns:
        Lista com os códigos dos níveis extraídos [CODNV0, CODNV1]
    """
    if not isinstance(codigo_completo, str) or len(codigo_completo) < 5:
        return None
    ...
    try:
        ...
        return [codnv0, codnv1]
    except:
        return None

def extrair_codcat1(codigo_completo):
    """
    Extrai o CODCAT1 que é a concatenação de CODNV0+CODNV1
    ...
    Returns:
        CODCAT1: Primeiros 5 caracteres do código ou código parcial se menor
    """
    if not isinstance(codigo_completo, str):
        return None
    
    # Código já é NV1 ou menor (até 5 caracteres)
    if len(codigo_completo) <= 5:
        return codigo_completo
        
    # Código é maior que NV1, truncar para 5 caracteres
    return codigo_completo[:5]  # Primeiros 5 caracteres

def calcular_pss(codigo_correto, codigo_score):
    """
    Calcula a pontuação por semelhança por score (pss).
    MODIFICAÇÃO: Agora suporta códigos de diferentes tamanhos
    ...
    Returns:
        Pontuação binária por nível e valor decimal (máximo 3)
    """
    # Extrair apenas os 5 primeiros caracteres (CODCAT1) para ambos códigos
    codigo_correto_nv1 = extrair_codcat1(codigo_correto)
    codigo_score_nv1 = extrair_codcat1(codigo_score)
    
    if codigo_correto_nv1 is None or codigo_score_nv1 is None:
        return [0, 0], 0
    
    # Verificar se conseguimos extrair os códigos individuais
    niveis_correto = extrair_codigos(codigo_correto_nv1) 
    niveis_score = extrair_codigos(codigo_score_nv1)
    ...
    if niveis_correto is None or niveis_score is None:
        ...
        binario = [0, 0]  # Começa com zeros
        
        # Se o código score começa com o mesmo caractere (CODNV0), marca o primeiro bit
        if len(codigo_correto_nv1) > 0 and len(codigo_score_nv1) > 0:
            if codigo_correto_nv1[0] == codigo_score_nv1[0]:
                binario[0] = 1
                
                # Se temos pelo menos 5 caracteres (NV1 completo) e eles são iguais
                if len(codigo_correto_nv1) >= 5 and len(codigo_score_nv1) >= 5:
                    if codigo_correto_nv1[:5] == codigo_score_nv1[:5]:
                        binario[1] = 1
    ...
    binario = []
    for i in range(2):  # Apenas 2 níveis para NV1
...
def calcular_ps(codigo_correto, codigo_scores):
    ...
    MODIFICAÇÃO: Usa nova escala baseada em 2 níveis (máximo 3)
    ...
    for i, cs in enumerate(codigo_scores):
        if cs:
            ...
            pss_resultados.append({
                'posicao': i+1,
                'codigo': None,
                'binario': [0, 0],  # Agora apenas 2 bits
                'decimal': 0,
                'peso': WSS[i],
                'contribuicao': 0
            })
...
def calcular_pp(codigo_correto, codigo_scores):
    ...
    MODIFICAÇÃO: Compara CODCAT1 com melhor robustez para diferentes formatos
    ...
    for i, cs in enumerate(codigo_scores):
        ...
        codigo_score_nv1 = extrair_codcat1(cs)
        
        if codigo_score_nv1 is None:
            continue
        
        # Comparar na maior profundidade disponível entre os dois códigos
        min_len = min(len(codigo_correto_nv1), len(codigo_score_nv1))
        
        # Para NV1 consideramos correto se os primeiros 5 caracteres são iguais (ou todos se menor)
        if min_len >= 5:  # Ambos têm pelo menos tamanho NV1 completo
            if codigo_correto_nv1[:5] == codigo_score_nv1[:5]:
                return PESOS_POSICAO[i], i+1
        elif min_len > 0:  # Comparação parcial
            if codigo_correto_nv1[:min_len] == codigo_score_nv1[:min_len]:
                return PESOS_POSICAO[i], i+1
...
def processar_output(output_file, samples_df):
    ...
    MODIFICAÇÃO: Usa o novo sistema de pontuação baseado em NV1 (2 níveis)
    """
    ...
    try:
        ...
        for _, sample in samples_df.iterrows():
            ...
            # MODIFICAÇÃO: Extrair CODCAT1 (5 primeiros caracteres)
            codigo_correto_nv1 = extrair_codcat1(codigo_correto)
            ...
            resultado = {
                ...
                'CODCAT1': codigo_correto_nv1,  # Novo campo CODCAT1
        ...
        console.print(f"  PS média (NV1): {stats['ps_media']:.4f}")
        console.print(f"  PP média (NV1): {stats['pp_media']:.4f}")
        console.print(f"  PG média (NV1): {stats['pg_media']:.4f}")
        console.print(f"  Acertos no TOP 1 (NV1): {stats['A_top1']} ({stats['A_top1_pct']:.2f}%)")
        console.print(f"  Acertos no TOP 5 (NV1): {stats['A_top5']} ({stats['A_top5_pct']:.2f}%)")
...
def main():
    ...
    MODIFICAÇÃO: Usa o novo diretório para resultados NV1 e exibe claramente que está usando o novo sistema
    """
    console.print("[bold magenta]===== INICIANDO AVALIAÇÃO DE MODELOS (VERSÃO NV1) =====[/bold magenta]")
    console.print("[bold yellow]ATENÇÃO: Esta versão avalia apenas os níveis 0 e 1 (ignorando NV2 e NV3)[/bold yellow]")
    
    # Verificar se o diretório de resultados existe, senão criar
    if not os.path.exists(RESULTADOS_DIR):
        os.makedirs(RESULTADOS_DIR)
        console.print(f"[yellow]Diretório de resultados NV1 criado: {RESULTADOS_DIR}[/yellow]")
    ...
    try:
        ...
        existing_results = glob.glob(os.path.join(RESULTADOS_DIR, "RESULTADOS_NV1_*.xlsx"))
        processed_files = set()
        
        for result_file in existing_results:
            # Extrair nome do arquivo original a partir do nome do resultado
            base_name = os.path.basename(result_file).replace("RESULTADOS_NV1_", "", 1)
            processed_files.add(base_name)
        
        console.print(f"[cyan]Encontrados {len(processed_files)} arquivos já processados com sistema NV1[/cyan]")
        ...
        if not files_to_process:
            ...
            for result_file in existing_results:
                try:
                    ...
                    stats = {
                        'arquivo': os.path.basename(result_file).replace("RESULTADOS_NV1_", "", 1),
            ...
            if todas_stats:
                ...
                stats_file = os.path.join(RESULTADOS_DIR, "RESUMO_ESTATISTICAS_NV1.xlsx")
                stats_df.to_excel(stats_file, index=False)
                console.print(f"[bold green]Resumo de estatísticas NV1 atualizado: {stats_file}[/bold green]")
                
                # Mostrar ranking
                console.print("\n[bold cyan]Ranking dos modelos por PG média (sistema NV1):[/bold cyan]")
        ...
        with Progress(
            ...
            task = progress.add_task("[blue]Processando novos arquivos com sistema NV1...", total=len(files_to_process))
            ...
            for output_file in files_to_process:
                ...
                if resultados_df is not None and len(resultados_df) > 0:
                    ...
                    base_name = os.path.basename(output_file)
                    result_file = os.path.join(RESULTADOS_DIR, f"RESULTADOS_NV1_{base_name}")
                    
                    # Salvar resultados
                    resultados_df.to_excel(result_file, index=False)
                    console.print(f"[green]Resultados NV1 salvos em: {result_file}[/green]")
        ...
        if novas_stats or existing_results:
            ...
            if os.path.exists(stats_file):
                try:
                    ...
                    console.print(f"[cyan]Carregadas {len(todas_stats)} estatísticas NV1 existentes[/cyan]")
                except Exception as e:
            ...
            if not todas_stats:
                console.print("[yellow]Reconstruindo estatísticas NV1 a partir dos resultados...[/yellow]")
                all_results = glob.glob(os.path.join(RESULTADOS_DIR, "RESULTADOS_NV1_*.xlsx"))
            ...
            if todas_stats:
                stats_df = pd.DataFrame(todas_stats)
                stats_df.to_excel(stats_file, index=False)
                console.print(f"[bold green]Resumo de estatísticas NV1 atualizado em: {stats_file}[/bold green]")
                ...
                for i, (_, row) in enumerate(ranked_stats.iterrows(), 1):
                    ...
                    score_gt70 = row['S>70'] if 'S>70' in row and pd.notna(row['S>70']) else 0
                    conf_gt50 = row['C>50'] if 'C>50' in row and pd.notna(row['C>50']) else 0
        ...
        console.print("[bold green]Processamento NV1 concluído com sucesso![/bold green]")
