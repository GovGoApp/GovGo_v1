 # Create descrição: combine only materialOuServico and descricao (explicitly NOT using objetoCompra)
...
OUTPUT_DIR = BASE_PATH + "CLASSY\\CLASSY_ITENS\\INPUT_ITENS_ALONE\\"
...
def extract_items():
    ...
    try:
        ...
        df['descrição'] = df.apply(
            lambda row: f"{('MATERIAL' if row['materialOuServico'] == 'M' else 'SERVIÇO')}:: {row['descricao']}",
            axis=1
        )
