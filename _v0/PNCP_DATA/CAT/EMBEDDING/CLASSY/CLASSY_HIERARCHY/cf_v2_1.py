...
PAGE_BEGIN = 41  # Página inicial para leitura do Excel
PAGE_END = 51 # Página final para leitura do Excel 
...
def partition_list_old(lst, n):
    k, m = divmod(len(lst), n)
    return [lst[i*k + min(i, m):(i+1)*k + min(i+1, m)] for i in range(n)]

def partition_list(lst, n):
    """Divide a lista em n partições o mais igualmente possível."""
    if not lst:
        return []
    ...
    for i in range(n):
        # Adicionar um item extra às primeiras 'remainder' partições
        partition_size = base_size + (1 if i < remainder else 0)
        end = start + partition_size
        
        if start < size:  # Evitar partições vazias
            partitions.append(lst[start:end])
        
        start = end
    
    # Verificação final para garantir que todos os itens foram incluídos
    total_items = sum(len(p) for p in partitions)
    if total_items != size:
        console.print(f"[bold red]ERRO: Tamanho total das partições ({total_items}) difere do tamanho da lista original ({size})[/bold red]")
...
def fast_hierarchical_beam_search_combined(item_embed, indices, cat_meta_by_level, cat_embeddings_by_level, hierarchical_maps):
    """Versão que combina cálculo hierárquico com cálculo direto."""
    
    # 1. Realizar cálculo hierárquico normal
    hierarchical_results = fast_hierarchical_beam_search(
        item_embed, indices, cat_meta_by_level, cat_embeddings_by_level, hierarchical_maps
    )
    
    # 2. Realizar cálculo direto
    #direct_results = calculate_direct_similarity(
    #    item_embed, cat_embeddings_by_level['NV4'], cat_meta_by_level['NV4']
    #)

    # 2. Realizar cálculo direto usando o índice FAISS existente
    direct_results = calculate_direct_similarity_with_faiss(
        item_embed, indices, cat_meta_by_level['NV4']
    )
    ...
    if OUTPUT_DEBUG and combined_results:
        # Pegar informações de debug do resultado hierárquico
        debug_info = {}
...
def calculate_direct_similarity_with_faiss(item_embed, indices, cat_meta_nv4):
    """Versão que usa o índice FAISS existente para busca direta."""
    # Normalizar embedding de consulta
    query_embed = np.array([item_embed], dtype=np.float32)
    faiss.normalize_L2(query_embed)
    
    # Usar índice FAISS existente
    D, I = indices['NV4'].search(query_embed, TOP_N)
    ...
    for i, (idx, distance) in enumerate(zip(I[0], D[0])):
        ...
        # Converter distância para similaridade (como já é IndexFlatIP, já é similaridade)
...
def classify_items_combined(df_items, indices, cat_meta_by_level, cat_embeddings_by_level, embedding_function):
    """Versão combinada da função classify_items que evita fragmentação do DataFrame."""
    ...
    # Em modo debug, adicionar colunas extras
    if OUTPUT_DEBUG:
        result_columns["SCORE_CALCULATION"] = [""] * num_rows
        
        # Adicionar colunas para scores de nível
        result_columns["SCORE_NV1"] = [0.0] * num_rows
        result_columns["SCORE_NV2"] = [0.0] * num_rows
        result_columns["SCORE_NV3"] = [0.0] * num_rows
        result_columns["SCORE_NV4"] = [0.0] * num_rows
        
        # Colunas para resultados diretos e hierárquicos
        for i in range(1, TOP_N + 1):
            result_columns[f"TOP_DIRECT_{i}"] = [""] * num_rows
            result_columns[f"SCORE_DIRECT_{i}"] = [0.0] * num_rows
            result_columns[f"TOP_HIER_{i}"] = [""] * num_rows
            result_columns[f"SCORE_HIER_{i}"] = [0.0] * num_rows
        
        # Colunas para resultados hierárquicos detalhados
        for i in range(1, TOP_NV1 + 1):
            result_columns[f"TOP_NV1_{i}"] = [""] * num_rows
            result_columns[f"SCORE_NV1_{i}"] = [0.0] * num_rows
        
        for i in range(1, TOP_NV2 + 1):
            result_columns[f"TOP_NV2_{i}"] = [""] * num_rows
            result_columns[f"SCORE_NV2_{i}"] = [0.0] * num_rows
            
        for i in range(1, TOP_NV3 + 1):
            result_columns[f"TOP_NV3_{i}"] = [""] * num_rows
            result_columns[f"SCORE_NV3_{i}"] = [0.0] * num_rows
    ...
    with progress:
        ...
        # Processar similaridade em paralelo
        args_list = [(idx, item_embed) for idx, item_embed in enumerate(item_embeds)]
        partitions = partition_list(args_list, MAX_WORKERS)
        similarity_results = []
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = []
            for worker_id, part in enumerate(partitions, start=1):
                task_id = progress.add_task(f"Worker {worker_id} similaridade combinada", total=len(part))
                futures.append(executor.submit(
    ...
    # Renomear coluna id se necessário
    if "id" in result_df.columns and "id_pncp" not in result_df.columns:
        result_df = result_df.rename(columns={"id": "id_pncp"})
    
    # Ordenar colunas para output
    if OUTPUT_DEBUG:
        # Definir ordem exata das colunas
        ordered_columns = [
            'id_pncp', 'objetoCompra',
        ...
        # Modo não-debug - apenas resultados combinados principais
        desired_columns = ['id_pncp', 'objetoCompra']
        for i in range(1, TOP_N + 1):
            desired_columns.append(f"TOP_{i}")
        for i in range(1, TOP_N + 1):
        ...
        # Adicionar coluna CONFIDENCE ao output padrão
...
def process_input_file(input_number):
    ...
    try:
        ...
        # Usar a nova função de classificação combinada
