#### CLASSSSSYYYY V3 - NO PARALLELLEL ####  
...
BASE_DIR = "C:\\Users\\Haroldo Duraes\\Desktop\\GOvGO\\v0\\#DATA\\PNCP\\"
NOVA_CAT_DIR = BASE_DIR + "CAT\\NOVA\\"
CLASS_DIR = BASE_DIR + "CLASSY\\"
CLASS_DIR = BASE_DIR + "CLASSY\\"
INPUT_DIR = CLASS_DIR + "INPUT\\"
OUTPUT_DIR = CLASS_DIR + "OUTPUT\\"
CHECKPOINT_DIR = CLASS_DIR + "CHECKPOINTS\\"

SHEET = "Sheet1"

NOVA_CAT_FILE = NOVA_CAT_DIR + "NOVA CAT.xlsx"
...
# BATCH e MAX_WORKERS
BATCH_SIZE = 100  # Tamanho do batch para processamento e salvamento
...
EMBEDDINGS_DIR = BASE_DIR + "EMBEDDING\\"
...
def classify_items_batched(df_items, cat_embeds, cat_meta, embedding_function):
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold blue]Calculando Similaridade..."), 
        ...
        for idx, item_embed in enumerate(item_embeds):
            ...
            for i, cat_idx in enumerate(top_indices):
                if i < TOP_N:  # Garantir que só usamos TOP_N categorias
                    ...
                    result_df.loc[idx, f"TOP_{i+1}"] = f"{codcat} - {nomcat}"
...
def process_input_file(input_number):
    ...
    INPUT_FILE = INPUT_DIR + f"\\INPUT_{input_num_str}.xlsx"
    OUTPUT_FILE = OUTPUT_DIR + f"OUTPUT_{input_num_str}_{TIMESTAMP}.xlsx"
    CHECKPOINT_FILE = CHECKPOINT_DIR + f"CHECKPOINT_{input_num_str}_{TIMESTAMP}.pkl"
...
def main():
    ...
    try:
        ...
        # Processar todos os arquivos INPUT_0XX.xlsx
        success_count = 0
        failure_count = 0
        
        # Procurar todos os arquivos INPUT_0XX.xlsx disponíveis
        input_files = []
        for i in range(2, 52):  # Processando do INPUT_002 ao INPUT_051
        ...
        # Processar cada arquivo encontrado sem usar Progress
        success_count = 0
        failure_count = 0
