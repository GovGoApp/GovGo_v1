...
TOP_BOX_STYLE = {
    ...
    'background-color': '#f9f9f9',
    'position': 'relative',
    'width': '100%',  # Ocupando 100% do espaço disponível
}
...
def reset_box_styles():
    """Retorna um dicionário com os estilos resetados para todos os boxes"""
    return {f'top-box-{i+1}': dict(TOP_BOX_STYLE) for i in range(5)}
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        # Coluna da esquerda - Descrição e Confiança (30%)
        dbc.Col([
            ...
            # ID Contratação movido para baixo da confiança
            html.Div(id="item-id-display", style=ID_STYLE)
        ], md=4),  # 4/12 ~ 33%
        
        # Coluna da direita - 5 quadros TOP (70%)
        dbc.Col([
            html.H4("Categorias Sugeridas (Clique para Selecionar)"),
            dbc.Row([
                # Criar 5 colunas para os boxes TOP_1 a TOP_5, com melhor distribuição
                dbc.Col([
                    html.Div([
                    ...
                    n_clicks=0,
                    style=TOP_BOX_STYLE)
                ], width=2, className="mx-0 px-1") for i in range(5)  # Reduzidos os espaçamentos
            ], className="g-0 mx-1"),  # g-0 remove o gutter entre colunas
...
], fluid=True)  # fluid=True para usar todo o espaço disponível

# Callback para inicializar e navegar pelos itens
@app.callback(
    [Output('item-data-store', 'data'),
     Output('item-description', 'children'),
     Output('item-id-display', 'children'),  # Adicionado para mostrar ID_ITEM_CONTRATACAO
...
def update_item_display(prev_clicks, next_clicks, key_submit, close_modal,
    ...
    if not current_data:
        ...
        if app.items_df is None:
            # Se o carregamento completo ainda não terminou, usar a amostra
    ...
    if app.preloaded and len(app.items_df) > len(sample_df):
        # Preservar o índice atual mas usar o DataFrame completo
        current_idx = app.current_item_index
        app.items_df = app.items_df  # Já está atualizado pelo thread
    ...
    if trigger in ['next-button', 'key-capture']:
        ...
        if current_data and current_selections:
            # Criar lista das categorias escolhidas em ordem
            ...
            # Adicionar seleções ao registro de validação com todos os campos originais
            validation_item = {
                'ID': current_data['ID'],
                ...
                'validated_choices_indices': current_selections,
...
for i in range(5):
    ...
    def toggle_selection(n_clicks, key_value, current_selections, current_style):
        # O resto do código dessa função permanece igual
        ...
        if ctx.triggered_id == 'key-capture' and key_value:
            # Processar teclas (números de 1 a 5 selecionam os boxes)
        ...
        # Caso não seja um clique no box específico ou tecla para este box, não faz nada
