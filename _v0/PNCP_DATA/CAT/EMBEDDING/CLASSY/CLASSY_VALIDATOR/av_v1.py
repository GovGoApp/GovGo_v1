...
def load_data(limit=1000):
    ...
    info_print(f"Conectando ao banco de dados: {DB_FILE}")
    
    try:
        with console.status("[bold green]Carregando dados do banco...", spinner="dots"):
            conn = sqlite3.connect(DB_FILE)
            
            # Use current timestamp to add randomness to the query
            ...
            ORDER BY (ID * {current_time}) % 100000000 + RANDOM()
            """
        ...
        success_print(f"Dados carregados: {len(df)} itens")
        
        # Exibir amostra dos dados
        if DEBUG and len(df) > 0:
            sample_table = Table(title="Amostra de Dados", show_header=True, header_style="bold")
            sample_table.add_column("ID_ITEM", style="cyan")
            sample_table.add_column("Descrição", style="green", max_width=500)
            sample_table.add_column("TOP_1", style="magenta", max_width=500)
        ...
        error_print(f"Erro ao carregar dados", e)
...
def build_prompt_old(item_data):
                ...
                if validated_choices_indices:
                    ...
                    for idx in validated_choices_indices:
                        if 0 <= idx <= 4:
                            ...
                            choices_text.append(f"{category[:1000]}... ", style="dim")
...
def main():
    console.print(Panel.fit(
        "[bold cyan]Auto-Validador de Classificações com CLASSY_VALIDATOR[/]", 
        border_style="blue",
        padding=(1, 10)
    ))
    
    # Perguntar se deseja limitar o número de itens
    limit = console.input("[bold yellow]Número de itens para processar (Enter para todos): [/]")
    ...
    if len(items_df) == 0:
        error_print("Nenhum item encontrado na base de dados!")
        return
    ...
    if success:
        ...
        with_validation = len([item for item in validation_results if item['validated_top_1']])
        ...
        avg_categories = sum(len(item['validated_choices_indices']) for item in validation_results) / len(validation_results)
