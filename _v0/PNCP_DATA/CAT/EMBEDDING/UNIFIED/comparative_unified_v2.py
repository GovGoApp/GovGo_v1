...
from emb_nv2_unified_v2 import (
...
providers_models = {
    ...
    "ollama": ["llama3.2", "bge-m3", "mxbai-embed-large"], #, ["bge-m3"], #["llama3.2"], # #["llama3.2", "bge-m3"],
...
for provider, models in providers_models.items():
    for model in models:
        ...
        # Parte onde são gerados os embeddings do catálogo
        try:
            ...
            if cat_embeds is None or len(cat_embeds) != len(cat_texts):
                # para {combo_name} não encontrados. Gerando novos...[/yellow]")
                cat_embeds = get_embeddings(cat_texts, method=provider, model=model, show_progress=True)
            ...
            df_results = classify_items_batched(
                ...
                lambda texts: get_embeddings(texts, method=provider, model=model, show_progress=True)
            )
