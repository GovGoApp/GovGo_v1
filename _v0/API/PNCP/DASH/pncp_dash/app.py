...
from utils import (
    ...
    summarize_documents, process_data, format_keywords, to_excel, get_excel_filename,
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardHeader("Filtros", className="filter-header small py-1"),
                dbc.CardBody([
                    # Input para palavras-chave
                    html.Div([
                        dbc.Label(html.B("Palavras-chave"), className="small mb-0"),
                        dbc.InputGroup([
                            dbc.Input(
                                id="keyword-input",
                            ...
                            dbc.Button("Add", id="add-keyword", n_clicks=0, size="sm")
                        ], size="sm"),
                        html.Div(id="keyword-chips", className="mt-1")
                    ], className="mb-2"),
                    
                    # Seletores de data - lado a lado
                    dbc.Row([
                        dbc.Col([
                            dbc.Label(html.B("Data Inicial"), className="small mb-0"),
                            dcc.DatePickerSingle(
                                id="data-inicial",
                                date=date(datetime.now().year, datetime.now().month, 1),
                                display_format="DD/MM/YYYY",
                                className="w-100 small-datepicker"
                            ),
                        ], width=6),
                        dbc.Col([
                            dbc.Label(html.B("Data Final"), className="small mb-0"),
                            dcc.DatePickerSingle(
                                id="data-final",
                                date=date.today(),
                                display_format="DD/MM/YYYY",
                    ...
                    html.Div([
                        dbc.Label(html.B("Modalidade"), className="small mb-0"),
                        dcc.Dropdown(
                            id="modalidade",
                    ...
                    # Seletor de UFs - apenas siglas
                    html.Div([
                        dbc.Label(html.B("Estados"), className="small mb-0"),
                        dcc.Dropdown(
                            id="ufs",
                            options=[{"label": k, "value": k} for k in get_ufs_brasil().keys()],
                            value=["ES"],
                            multi=True,
                            className="small-dropdown"
                        )
                    ...
                    # Tamanho da página
                    html.Div([
                        dbc.Label(html.B("Itens por Página"), className="small mb-0"),
                        dbc.Input(
                            id="tamanho-pagina",
                            type="number",
                            min=10,
        ...
        dbc.Col([
           # Área de filtros ativos e botão de exportação
            dbc.Row([
                dbc.Col([
                    dbc.Row([
                        dbc.Col([
                            html.Span(html.B("Filtros Ativos:"), className="me-2 small"),
                            html.Div(id="filtros-ativos", className="d-inline-block"),
                        ], width=10),
                        dbc.Col([
                            dbc.Button(
                                "Exportar Excel", 
...
def render_filtros_ativos(keywords, data_inicial, data_final, modalidade, ufs):
    ...
    # Palavras-chave
    if keywords:
        filtros.append(html.Div(f"Keywords: {', '.join(keywords)}", className="filter-chip"))
    ...
    if data_inicial or data_final:
        ...
        filtros.append(html.Div(f"Período: {data_ini} até {data_fim}", className="filter-chip"))
    
    # Modalidade
    if modalidade:
        nome_modalidade = next((k for k, v in get_modalidades().items() if v == modalidade), "Desconhecida")
        # Pegar apenas a parte depois do número
        nome_curto = nome_modalidade.split(" - ")[-1] if " - " in nome_modalidade else nome_modalidade
        filtros.append(html.Div(f"Modalidade: {nome_curto}", className="filter-chip"))
    
    # UFs
    if ufs:
        filtros.append(html.Div(f"UFs: {', '.join(ufs)}", className="filter-chip"))
    
    if not filtros:
        return html.Span("Nenhum filtro aplicado", className="text-muted small")
    
    return html.Div(filtros, className="d-inline")
...
@callback(
    Output({"type": "topicos-content", "index": MATCH}, "children"),  # Agora atualizamos o conteúdo
    Output({"type": "topicos-card", "index": MATCH}, "className"),    # Ainda controlamos a visibilidade
...
def gerar_topicos(n_clicks, processo):
    ...
    if not objeto:
        return "Objeto não disponível", "topicos-card", ""
    ...
    try:
        ...
        # Retorna os chips e torna card visível removendo d-none
        return topicos_chips, "topicos-card", ""
    except Exception as e:
        return f"Erro: {str(e)}", "topicos-card", ""
        ...
        card = dbc.Row([
            ...
            dbc.Col([
                dbc.Card([
                    ...
                    dbc.CardHeader([
                        # Lado esquerdo: UF
                        html.Span(f"{uf}", className="badge bg-light text-dark"),
                        
                        # Centro: Nome do órgão (flex-grow-1 faz ocupar o espaço disponível)
                        html.H5(orgao, className="mb-0 mx-2 text-center flex-grow-1"),
                        ...
                        html.Div([
                            ...
                            dbc.Button(
                                "Sumarizar", 
                                id={"type": "btn-sumarizar-todos", "index": idx},
                                color="primary",
                                size="sm"
                            ),
                        ], className="d-flex")
                    ...
                    dbc.CardBody([
                        ...
                        html.Div([
                            ...
                            # Substituir as linhas de exibição do objeto 
                            html.P([html.Strong("Objeto: "), html.Span(objeto)], className="mb-1"),

                            # Spinner para carregamento de tópicos (manter)
                            html.Div(
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Div([
                                        html.Div(
                                            [
                                                # Apenas o link para o documento
                                                html.A(doc.get("titulo", "Documento"), 
                                                    href=doc.get("url", "#"), 
                                                    target="_blank")
                                            ],
                                            className="documento-item mb-1 py-1",
                                        ) for i, doc in enumerate(documentos)
                                    ])
                                ],
            ...
            dbc.Col([
                # Card Tópicos
                    html.Div(
                        [
                            html.Div("Tópicos do Processo", className="topicos-header"),
                            dbc.Spinner(
                                html.Div(
                                    id={"type": "topicos-content", "index": idx},
                                    className="p-3"
                                ),
                                size="sm",
                                color="primary",
                                delay_show=300  # opcional: tempo em ms antes de mostrar o spinner
                            )
                        ],
                        id={"type": "topicos-card", "index": idx},
                        className="d-none topicos-card"  # começa oculto
                    ),
                ...
                html.Div(
                    [
                        html.Div("Sumário dos Documentos", className="topicos-header"),
                        dbc.Spinner(
                            html.Div(
                                id={"type": "sumario-content", "index": idx},
                                className="p-3"
                            ),
                            size="sm",
                            color="primary",
                    ...
                    id={"type": "sumario-card", "index": idx},
                    className="d-none sumario-card",
                    style={"marginTop": "10px"}
                )

            ], md=4, className="d-flex flex-column align-items-stretch" ) # Forçar alinhamento superior na coluna
...
@callback(
    Output({"type": "sumario-content", "index": MATCH}, "children"),
    Output({"type": "sumario-card", "index": MATCH}, "className"),
    Input({"type": "btn-sumarizar-todos", "index": MATCH}, "n_clicks"),
    State({"type": "processo-store", "index": MATCH}, "data"),
    prevent_initial_call=True
)
def exibir_sumario(n_clicks, processo):
    if not n_clicks or not processo:
    ...
    if not (cnpj and ano and sequencial):
        return "Dados insuficientes para buscar documentos.", "sumario-card"
    
    # Busca todos os documentos do processo
    documentos = fetch_documentos(cnpj, ano, sequencial)
    if not documentos:
        return "Nenhum documento encontrado para este processo.", "sumario-card"
    
    # Gera sumário usando a função do utils
    summary = summarize_documents(documentos)
    
    # Retorna o sumário e remove a classe "d-none"
    return summary, "sumario-card"
