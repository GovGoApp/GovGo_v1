...
# Ativa com --debug ou variável de ambiente DEBUG=1/true
...
try:
    if api_key:
        ...
        try:
            ...
            thread = None
            debug_print(f"Falha ao criar thread OpenAI: {e}")
    else:
        client = None
        thread = None
        debug_print("OPENAI_API_KEY ausente no .env local. Configure em 'OPENAI_API_KEY'.")
except Exception as e:
    client = None
    thread = None
...
def get_assistant_response(user_query: str):
    ...
    try:
        ok_cfg, _ = validate_openai_config()
        if not ok_cfg:
            return None
        if not client:
            debug_print("OpenAI client não inicializado")
            return None
        if not assistant_id:
            debug_print("Assistant ID ausente")
            return None
        global thread
        if thread is None:
            try:
                ...
                debug_print(f"Falha ao criar thread na hora: {e}")
                return None
...
def b64_image(image_path):
    """Resolve o src da imagem para o Dash.

    - Se for URL (http/https), retorna a própria URL.
    - Se for caminho local, retorna data URI em base64.
...
header = html.Div([
    ...
    html.Div([
        ...
        html.H4("Supabase Reports v1", style={'marginLeft': '15px', 'color': '#003A70'})
    ], style={'display': 'flex', 'alignItems': 'center'}),
    
    # Ícones à direita - mantidos como estavam
    html.Div([
        html.I(className="fas fa-list", style={
            'fontSize': '20px', 
            'marginRight': '15px', 
            'cursor': 'pointer',
            'color': '#FF5722'
...
# Ajustar o layout principal para incluir o cabeçalho
app.layout = html.Div([
...
def process_query(is_processing, query, conversation, counter):
    ...
    try:
        # Valida config do OpenAI antes de tentar
        ok_cfg, cfg_msg = validate_openai_config()
        if not ok_cfg:
            new_message = {
                'id': str(counter),
                'type': 'user',
            ...
            return updated_conversation, {}, counter + 1, False
        ...
        if not sql_query:
            ...
            return updated_conversation, {}, next_counter, False
        ...
        return updated_conversation, df_dict, next_counter, False  # Valor booleano literal
        ...
        return updated_conversation, {}, counter, False
