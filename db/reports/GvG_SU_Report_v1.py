...
# Variável de controle para debug - pode ser ativada via argumento --debug
...
def render_results(results):
    ...
    for result in results:
        try:
            debug_print(f"Processando resultado {result['id']}: data = {str(result['data'])[:100]}")
            ...
            debug_print(f"Resultado {result['id']}: df_data = {str(df_data)[:100]}..., len = {len(df_data) if df_data else 'None'}")
