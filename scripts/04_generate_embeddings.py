# =======================================================================
# [4] GERAÇÃO DE EMBEDDINGS ULTRA OTIMIZADA - VERSÃO V1
# =======================================================================
# Versão ultra otimizada do script 04C com melhorias de performance críticas
# 
# CORREÇÕES PRINCIPAIS V4 (mudança mínima):
...
# - Batch por tokens (~chars/4) com teto por requisição
# - RateLimiter tornado NO-OP (sem espera/bloqueio)
# - INSERT: batch_size=500, page_size=200 (menos round-trips)
...
# Configurações de embedding ULTRA OTIMIZADAS V4 - CORRIGIDAS
EMBEDDING_MODEL = "text-embedding-3-large"
EMBEDDING_DIM = 3072
MIN_WORKERS = 5
MAX_WORKERS = 20   # REDUZIDO: Menos contenção, mais estabilidade
...
class RateLimiter:
    """Rate limiter para API OpenAI — NO-OP nesta versão 04 (sem espera/bloqueio)."""
...
def setup_logging(debug_mode=False):
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("[2/3] GERAÇÃO DE EMBEDDINGS")
    logger.info("=" * 80)
    logger.info(f"Modo debug: {'Ativado' if debug_mode else 'Desativado'}")
    logger.info(f"Workers={MAX_WORKERS}, BatchSize={BATCH_SIZE}, Pool={NUM_CONN} conexões")
    logger.info("-" * 80)
...
def log_session_end(stats, tempo_total):
    ...
    if logger:
        logger.info("-" * 80)
        logger.info("GERAÇÃO DE EMBEDDINGS FINALIZADA")
...
# Configurações do banco PostgreSQL V1 (Supabase) - ULTRA OTIMIZADAS V4
DB_CONFIG = {
...
def initialize_connection_pool():
    """Inicializa pool de conexões thread-safe ULTRA OTIMIZADO V4"""
    global connection_pool
    try:
        # CORREÇÃO V4: Pool menor e mais estável
...
def get_db_connection():
    """Context manager para obter conexão do pool com tratamento robusto V4"""
    global connection_pool
...
def calculate_optimal_batch_size(texts):
    """Calcula batch size ótimo por orçamento de tokens (~chars/4).
    Mantém mudanças mínimas e limita o lote para caber com folga no request.
    """
    if not texts:
        return BATCH_SIZE
    avg_chars = sum(len(str(t)) for t in texts) / len(texts)
    avg_toks = max(1, int(avg_chars // 4))
    max_req_tokens = int(os.getenv("OPENAI_MAX_TOKENS_PER_REQ", "7600"))
    by_token = max(1, max_req_tokens // avg_toks)
    return min(len(texts), max(1, min(50, by_token)))
...
def get_db_connection_for_insert():
    """Context manager para inserções com timeout maior - CORRIGIDO V4"""
    global connection_pool
    if not connection_pool:
        raise Exception("Pool de conexões não inicializado")
...
def insert_embeddings_batch_optimized(contratacoes, embeddings, worker_id):
    """Insere embeddings - ULTRA OTIMIZADO V4"""
    ...
    while retry_count < max_retries:
        try:
            with get_db_connection_for_insert() as conn:
                ...
                # Preparar dados para inserção - METADATA JSON SIMPLIFICADO V4
                insert_data = []
                ...
                batch_size = min(500, len(insert_data))  # Lotes de no máximo 100
                total_inserted = 0
                ...
                for i in range(0, len(insert_data), batch_size):
                    ...
                    execute_values(
                        ...
                        page_size=200  # Page size menor para estabilidade
                    )
...
def main():
    ...
    console.print(Panel("[bold blue][2/3] GERAÇÃO DE EMBEDDINGS[/bold blue]"))
    ...
    try:
        ...
        if global_stats['erros'] == 0:
            ...
            console.print("[bold yellow]⚠️ PROCESSAMENTO CONCLUÍDO COM ALGUNS ERROS[/bold yellow]")
    
    except KeyboardInterrupt:
        log_message("⚠️ Script interrompido pelo usuário (Ctrl+C)", "warning")
        console.print("[bold yellow]⚠️ PROCESSAMENTO INTERROMPIDO PELO USUÁRIO[/bold yellow]")
    except Exception as e:
        log_message(f"❌ Erro crítico no processamento: {e}", "error")
        console.print("[bold red]❌ ERRO CRÍTICO NO PROCESSAMENTO[/bold red]")
    finally:
