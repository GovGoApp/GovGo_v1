...
def convert_document_to_markdown(file_path, original_filename):
    # Prefer running conversion in a subprocess to isolate native crashes
    try:
        if os.environ.get('GVG_DOC_SUBPROC', '1') == '1':
            try:
        ...
        # Fallback para execução inline (se desativado por variável de ambiente)
        try:
            print(f"[GSB][RESUMO] convert_document_to_markdown() iniciar imports para '{original_filename}'")
        except Exception:
            pass
        from docling.document_converter import DocumentConverter, PdfFormatOption
        ...
        from docling.backend.pypdfium2_backend import PyPdfiumDocumentBackend
        try:
            print("[GSB][RESUMO] convert_document_to_markdown() imports OK")
        except Exception:
            pass
        ...
        try:
            print("[GSB][RESUMO] Instanciando DocumentConverter...")
        except Exception:
            pass
        ...
        try:
            print("[GSB][RESUMO] Chamando doc_converter.convert()...")
        except Exception:
            pass
        result = doc_converter.convert(file_path)
        try:
            print("[GSB][RESUMO] doc_converter.convert() OK")
        except Exception:
            pass
        doc = result.document
        print("✅ Conversão concluída, extraindo markdown...")
        try:
            print("[GSB][RESUMO] Exportando markdown...")
        except Exception:
            pass
...
def generate_document_summary(markdown_content, max_tokens=None, pncp_data=None):
    try:
        try:
            print("[GSB][RESUMO] generate_document_summary() iniciando...")
        except Exception:
            pass
...
"""
        ...
        try:
            print("[GSB][RESUMO] generate_document_summary() OK")
        except Exception:
            pass
