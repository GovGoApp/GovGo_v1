...
# Utilitário de log unificado (respeita GVG_SQL_DEBUG e DEBUG_INTELLIGENT_QUERIES)
...
def semantic_search(query_text,
	...
	try:
		...
		# Ajustes de sessão para IVFFLAT nas fontes com halfvec (contrato e contratacao)
		try:
			if get_current_source() in ('contrato','contratacao'):
				try:
					...
					probes = 32
				# SET LOCAL garante escopo da transação atual
				cursor.execute(f"SET LOCAL ivfflat.probes = {probes}")
				cursor.execute("SET LOCAL enable_seqscan = off")
		except Exception as _sess_err:
			_log(f"⚠️ Não foi possível ajustar sessão IVFFLAT: {_sess_err}")

		vector_opt_enabled = os.getenv("GVG_VECTOR_OPT", "1") != "0"
		# Para 'contrato' e 'contratacao' com halfvec, preferimos kNN direta (evita CTE pré-filtro)
		if get_current_source() in ('contrato','contratacao'):
		...
		if vector_opt_enabled:
			try:
				...
				# Distância calculada sobre a expressão de coluna com cast adequado
				...
				_log(f"⚠️ Vetor otimizado falhou: {opt_err}")
				try:
					if conn:
						conn.rollback()
				except Exception:
...
def hybrid_search(query_text, limit=MAX_RESULTS, min_results=MIN_RESULTS,
	...
	try:
		...
		core_cols = get_core_columns('c')  # dinâmico
		text_field = f"c.{get_fts_source_field()}"
