...
Nota v2: Esta versão permite alternar a fonte de dados entre 'contratação' e 'contrato' pelo menu principal (opção 0).
"""
...
try:
    ...
    from gvg_schema import set_current_source, get_current_source
...
SORT_MODES = {
    1: {"name": "Similaridade", "description": "Ordenar por índice de proximidade", "emoji": "🎯"},
    2: {"name": "Data", "description": "Ordenar por data de publicação", "emoji": "📅"},
...
def display_menu():
    ...
    # Opção 0: Alterar fonte de dados diretamente no menu principal
    try:
        current_source = get_current_source()
    except Exception:
        current_source = 'contrato'
    console.print(f"0. Fonte de dados: [bold]{current_source}[/bold] (alterar)")
    ...
    while True:
        try:
            ...
            # Opções do menu
                ...
                # Alternância direta (toggle) da fonte de dados
                try:
                    current_source = get_current_source()
                except Exception:
                    current_source = 'contrato'
                new_src = 'contratacao' if current_source == 'contrato' else 'contrato'
                try:
                    set_current_source(new_src)
                    console.print(f"✅ Fonte alterada para: {new_src}")
                except Exception as e:
                    console.print(f"❌ Não foi possível alterar a fonte: {e}")
