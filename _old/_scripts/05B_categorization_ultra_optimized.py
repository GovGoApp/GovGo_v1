# =======================================================================
# [5B/7] CATEGORIZAÇÃO AUTOMÁTICA ULTRA OTIMIZADA - VERSÃO V1
# =======================================================================
# Versão ultra otimizada do script 05 com melhorias de performance críticas
# baseadas nas otimizações implementadas no 04E_embedding_generation_optimized.py
# 
# PRINCIPAIS OTIMIZAÇÕES V5B:
# - Pool de conexões thread-safe (ThreadedConnectionPool)
# - Timeouts diferenciados para SELECTs e UPDATEs
# - Query inicial otimizada sem JOINs pesados
...
# - Modo incremental diário (como 04E)
# - Modo --test para próximo dia
# - Busca semântica com pgvector
# - Cálculo de confidence score
# - Atualização de system_config
# 
# Resultado: Performance drasticamente melhorada para dias com muitas contratações
# =======================================================================
...
# Configurações de categorização ULTRA OTIMIZADAS V5B
TOP_K = 5
MIN_WORKERS = 3
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Categorização Automática Ultra Otimizada v1')
...
def setup_logging(debug_mode=False):
    ...
    LOG_FILE = os.path.join(logs_dir, f"05B_categorization_ultra_optimized_{start_timestamp}.log")
    
    # Configurar logger principal
    logger = logging.getLogger('categorization_ultra_optimized')
    ...
    logger.info("=" * 80)
    logger.info("NOVA SESSÃO DE CATEGORIZAÇÃO ULTRA OTIMIZADA V5B INICIADA")
    logger.info("=" * 80)
    ...
    logger.info(f"OTIMIZAÇÕES V5B: Workers={MAX_WORKERS}, BatchSize={BATCH_SIZE}, Pool={NUM_CONN} conexões")
    logger.info("-" * 80)
...
def log_session_end(stats, tempo_total):
    ...
    if logger:
        logger.info("-" * 80)
        logger.info("SESSÃO DE CATEGORIZAÇÃO ULTRA OTIMIZADA V5B FINALIZADA")
        ...
        logger.info("OTIMIZAÇÕES V5B APLICADAS: Pool conexões, timeouts otimizados, batch updates, query sem JOINs")
        logger.info("=" * 80)
...
# Configurações do banco PostgreSQL V1 (Supabase) - ULTRA OTIMIZADAS V5B
DB_CONFIG = {
...
#############################################
# POOL DE CONEXÕES OTIMIZADO V5B
#############################################

def initialize_connection_pool():
    """Inicializa pool de conexões thread-safe ULTRA OTIMIZADO V5B"""
    ...
    try:
        ...
        log_message(f"Pool de conexões OTIMIZADO criado: {NUM_CONN} conexões máximas", "success")
        return True
...
def get_db_connection(for_update=False):
    """Context manager para obter conexão do pool com tratamento robusto V5B"""
    global connection_pool
        ...
        try:
            ...
            if conn and not conn.closed:
                ...
                with conn.cursor() as test_cursor:
                    ...
                    # Configurar timeout específico para o tipo de operação
                    if for_update:
                        test_cursor.execute("SET statement_timeout = 180000")  # 3 minutos para updates
                    else:
                        test_cursor.execute("SET statement_timeout = 120000")   # 2 minutos para selects (aumentado)
                yield conn
                break
            else:
                if conn:
...
#############################################
# FUNÇÕES DE BANCO DE DADOS ULTRA OTIMIZADAS V5B
#############################################
...
def get_embeddings_by_date_optimized(date_str):
    """Obtém embeddings de uma data específica - QUERY SIMPLIFICADA SEM VERIFICAÇÕES EXTRAS"""
    try:
        with get_db_connection() as conn:
            ...
            # QUERY SIMPLIFICADA: apenas buscar embeddings da data sem verificações extras
            query = """
            SELECT 
...
#############################################
# FUNÇÕES DE CATEGORIZAÇÃO OTIMIZADAS V5B
#############################################
...
def categorize_embedding_batch_optimized(worker_id, embedding_batch, progress_callback=None):
    """Categoriza um lote de embeddings - ULTRA OTIMIZADO V5B com batch updates"""
    local_stats = {
        'embeddings_processados': 0,
        'categorizacoes_aplicadas': 0,
        'baixa_confianca': 0,
    ...
    try:
        ...
        # OTIMIZAÇÃO V5B: Processar em sub-lotes para batch updates
        update_data = []
        processed_count = 0
        ...
        with get_db_connection(for_update=True) as conn:
            ...
            # Processar embeddings em lotes - SEM VERIFICAÇÕES DE DUPLICAÇÃO
            for i, embedding_data in enumerate(embedding_batch):
                try:
                    ...
                    log_message(f"Worker {worker_id}: Erro ao processar embedding {id_emb}: {e}", "error")
                    local_stats['erros'] += 1
            
            # OTIMIZAÇÃO V5B: Batch update usando execute_values
            if update_data:
                if progress_callback:
                ...
                conn.commit()
                log_message(f"🔧 Worker {worker_id}: Batch update concluído - {len(update_data)} registros atualizados", "debug")
            
            if progress_callback:
                progress_callback(100)
        
        log_message(f"🔧 Worker {worker_id}: Concluído - {local_stats['categorizacoes_aplicadas']} categorizações de {local_stats['embeddings_processados']} embeddings", "debug")
...
def process_categorization_for_date(date_str):
    """Processa categorização para todas as contratações de uma data específica - OTIMIZADO V5B"""
    log_message(f"Processando categorização para data: {format_date_for_display(date_str)}")
    
    # Obter embeddings da data usando query simplificada
    embeddings = get_embeddings_by_date_optimized(date_str)
    ...
    log_message(f"Data {format_date_for_display(date_str)}: processando {len(embeddings)} embeddings")
    ...
    # PROCESSAMENTO OTIMIZADO com progress tracking
    if debug_mode:
        ...
        with Progress(
            ...
            main_task = progress.add_task(
                f"[bold blue]📊 ULTRA DEBUG V5B: Data {date_str}", 
            ...
            for i in range(len(partitions)):
                ...
                worker_tasks[worker_id] = progress.add_task(
                    f"[cyan]🔧 Worker {worker_id}: {partition_size} embeddings",
                    total=100
                )
            ...
            def update_debug_progress():
                ...
                progress.update(main_task, 
                    ...
                    description=f"[bold blue]📊 ULTRA DEBUG V5B: Data {date_str} ({len(partitions)} workers)"
                )
            ...
            def process_with_debug_progress(worker_id, partition, date_str):
                def worker_progress_callback(completed_percent):
                    ...
                    progress.update(worker_tasks[worker_id], 
                        ...
                        description=f"[cyan]🔧 Worker {worker_id}: {completed_percent}% ({len(partition)} embeddings)"
                    )
                    update_debug_progress()
                
                try:
                    result = categorize_embedding_batch_optimized(
                        worker_id, partition, worker_progress_callback
                    )
                    progress.update(worker_tasks[worker_id], 
                        completed=100,
                        description=f"[green]✅ Worker {worker_id}: Concluído ({len(partition)} embeddings)"
                    )
                    return result
                except Exception as e:
                    progress.update(worker_tasks[worker_id], 
        ...
        with Progress(
            ...
            main_task = progress.add_task(
                f"[bold blue]📊 ULTRA OTIMIZADA V5B: Data {date_str}", 
            ...
            def update_general_progress():
                ...
                progress.update(main_task, 
                    completed=total_progress,
                    description=f"[bold blue]📊 ULTRA OTIMIZADA V5B: Data {date_str} ({len(partitions)} workers)"
                )
    ...
    log_message(f"✅ Data {date_display} concluída: {date_stats['categorizacoes_aplicadas']} categorizações, {date_stats['erros']} erros", 
                "success" if date_stats['erros'] == 0 else "warning")
    
    return date_stats

#############################################
# FUNÇÃO PRINCIPAL ULTRA OTIMIZADA V5B
#############################################

def main():
    """Função principal da categorização automática ULTRA OTIMIZADA V5B"""
    global debug_mode
    ...
    console.print(Panel("[bold blue][5B/7] CATEGORIZAÇÃO AUTOMÁTICA ULTRA OTIMIZADA V1[/bold blue]"))
    
    log_message(f"Arquivo de log criado: {log_filename}", "info")
    log_message(f"Configurações OTIMIZADAS V5B: {MAX_WORKERS} workers, batch size {BATCH_SIZE}, pool {NUM_CONN} conexões", "info")
    ...
    try:
        ...
        for date_str in dates_to_process:
            try:
                ...
                # Processar categorização para a data
                date_stats = process_categorization_for_date(date_str)
                ...
                if date_stats['erros'] == 0:
                    ...
                    log_message(f"✅ Data {date_display} concluída com sucesso: {date_stats['categorizacoes_aplicadas']} categorizações", "success")
                else:
                    log_message(f"⚠️  Data {date_display} concluída com {date_stats['erros']} erros - data NÃO atualizada para permitir reprocessamento", "warning")
        ...
        log_message(f"Categorização ULTRA OTIMIZADA V5B concluída em {tempo_total:.1f}s", "success")
        ...
        if global_stats_total['erros'] == 0:
            console.print(Panel("[bold green]✅ CATEGORIZAÇÃO ULTRA OTIMIZADA V5B CONCLUÍDA[/bold green]"))
        else:
            console.print(Panel("[bold yellow]⚠️ CATEGORIZAÇÃO V5B CONCLUÍDA COM ALGUNS ERROS[/bold yellow]"))
