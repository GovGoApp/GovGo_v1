...
def insert_itens(conn, itens_data):
    ...
    try:
        ...
        try:
            ...
            inserted_count = 0
            for record in itens_data:
                # Verificar se já existe (convertendo numero_item para string)
                cursor.execute(
                    "SELECT COUNT(*) FROM item_contratacao WHERE numero_controle_pncp = %s AND numero_item = %s",
                    (record['numero_controle_pncp'], str(record['numero_item']))
                )
...
def get_last_processed_date(conn=None):
    """Retorna a última data processada do arquivo de log ou da base de dados"""
    
    # Primeiro, tentar buscar da base de dados se houver conexão
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("""
            ...
            if result and result[0]:
                ...
                log_message(f"Última data encontrada na base: {date_from_db}", "info")
                return date_from_db
        except Exception as e:
            log_message(f"Erro ao buscar data da base: {str(e)}", "warning")
    
    # Se não conseguir da base, tentar do arquivo de log
    if os.path.exists(LAST_DATE_FILE):
        with open(LAST_DATE_FILE, 'r') as f:
            last_date = f.read().strip()
            if re.match(r'^20\d{6}$', last_date):
                log_message(f"Última data processada encontrada no arquivo: {last_date}", "info")
                return last_date
    
    # Se não encontrar em lugar nenhum, usar data padrão recente
...
def main():
    ...
    try:
        ...
        if test_mode:
            ...
            date_range = get_date_range(conn)  # Passa a conexão
