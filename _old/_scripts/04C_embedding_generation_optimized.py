# =======================================================================
# [4C/7] GERAÇÃO DE EMBEDDINGS OTIMIZADA - VERSÃO V1
# =======================================================================
# Versão otimizada do script 04B com correção de performance crítica
# 
# CORREÇÃO PRINCIPAL V4C:
# - Eliminada query pesada get_dates_needing_embeddings()
# - Substituída por loop diário simples (last_embedding_date -> last_processed_date)
# - Performance instantânea ao invés de timeout de 6 meses
# 
# PRINCIPAIS OTIMIZAÇÕES MANTIDAS:
# - Pool de conexões reutilizáveis para evitar timeouts SSL
# - Verificação de duplicatas por lote (não global)
# - Rate limiting inteligente para API OpenAI
# - Circuit breaker para recuperação de erros
# - Workers reduzidos para evitar saturação
...
EMBEDDING_DIM = 3072
MAX_WORKERS = 20  # REDUZIDO de 16 para 20
BATCH_SIZE = 50   # REDUZIDO de 50 para 50
...
class RateLimiter:
    """Rate limiter inteligente para API OpenAI"""
    def __init__(self, rpm=3000):  # Margem de segurança (3500 real)
        self.rpm = rpm
        self.calls = []
    ...
    def wait_if_needed(self):
        ...
        with self.lock:
            ...
            if len(self.calls) >= self.rpm:
                sleep_time = 60 - (now - self.calls[0]) + 0.1  # Margem extra
                if sleep_time > 0:
...
def setup_logging(debug_mode=False):
    ...
    LOG_FILE = os.path.join(logs_dir, f"04C_embeddings_optimized_{start_timestamp}.log")
    
    # Configurar logger principal
    logger = logging.getLogger('embedding_generation_optimized')
    ...
    logger.info("=" * 80)
    logger.info("NOVA SESSÃO DE GERAÇÃO DE EMBEDDINGS OTIMIZADA V4C INICIADA")
    logger.info("=" * 80)
...
def log_session_end(stats, tempo_total):
    ...
    if logger:
        logger.info("-" * 80)
        logger.info("SESSÃO DE GERAÇÃO DE EMBEDDINGS OTIMIZADA V4C FINALIZADA")
...
# Configurações do banco PostgreSQL V1 (Supabase) - OTIMIZADAS
DB_CONFIG = {
...
def initialize_connection_pool():
    """Inicializa pool de conexões thread-safe"""
    global connection_pool
    try:
        connection_pool = psycopg2.pool.ThreadedConnectionPool(
            minconn=1,
            maxconn=MAX_WORKERS + 2,  # Margem extra para operações principais
            **DB_CONFIG
        )
        log_message(f"Pool de conexões criado: {MAX_WORKERS + 2} conexões máximas", "success")
        return True
    except Exception as e:
...
def get_db_connection():
    """Context manager para obter conexão do pool com tratamento robusto"""
    global connection_pool
    ...
    try:
        ...
        if conn and conn.closed:
            log_message("Conexão fechada detectada, reconectando...", "warning")
            connection_pool.putconn(conn, close=True)
            conn = connection_pool.getconn()
        yield conn
    except Exception as e:
        if conn:
            # Em caso de erro, marcar conexão como inválida
            try:
                if not conn.closed:
                    conn.rollback()
            except:
        ...
        if conn:
            try:
                # Sempre devolver conexão ao pool, mesmo em caso de erro
...
def create_embedding_constraints_if_not_exists():
...
def get_contratacoes_by_date(date_str):
    ...
    try:
        with get_db_connection() as conn:
            ...
            # QUERY OTIMIZADA: Primeiro filtra contratos sem embeddings, depois busca itens
            query = """
            SELECT 
                c.numero_controle_pncp,
                c.objeto_compra,
                COALESCE(
                    (SELECT string_agg(COALESCE(i.descricao_item, ''), ' :: ')
                     FROM item_contratacao i 
                     WHERE i.numero_controle_pncp = c.numero_controle_pncp), 
                    ''
                ) as itens_concatenados
            FROM contratacao c
            WHERE c.data_publicacao_pncp IS NOT NULL 
              AND DATE(c.data_publicacao_pncp) = %s::date
              AND NOT EXISTS (
...
#############################################
# FUNÇÕES DE EMBEDDING OTIMIZADAS
#############################################

def check_batch_duplicates(numero_controles):
    """OTIMIZADO: Verifica duplicatas apenas do lote atual (não global)"""
    ...
    try:
        with get_db_connection() as conn:
            ...
            """, (list(numero_controles),))
            return set(row[0] for row in cursor.fetchall())
        
    except Exception as e:
        log_message(f"Erro ao verificar duplicatas do lote: {e}", "error")
        return set()

def calculate_optimal_batch_size(texts):
    """Calcula batch size ótimo baseado no tamanho dos textos"""
    if not texts:
        return BATCH_SIZE
    ...
    if avg_length > 4000:
        return min(15, len(texts))
    elif avg_length > 2000:
        return min(25, len(texts))
    else:
        return min(50, len(texts))
...
def process_embedding_batch_optimized(worker_id, batch_contratacoes, date_str, progress_callback=None):
    """Processa um lote de contratações com todas as otimizações"""
    local_stats = {
        'contratacoes_processadas': 0,
        'embeddings_gerados': 0,
        'embeddings_cache': 0,
    ...
    try:
        ...
        # OTIMIZAÇÃO: Verificar duplicatas apenas do lote atual
        numero_controles = [c['numero_controle_pncp'] for c in batch_contratacoes]
        existing_embeddings = check_batch_duplicates(numero_controles)
        ...
        for i in range(0, len(to_process), optimal_batch_size):
            batch_to_process = to_process[i:i + optimal_batch_size]
...
def insert_embeddings_batch_optimized(contratacoes, embeddings, worker_id):
    """Insere embeddings com pool de conexões otimizado"""
    ...
    try:
        with get_db_connection() as conn:
            ...
            for contratacao, embedding in zip(contratacoes, embeddings):
                ...
                descricao_completa = str(contratacao.get('objeto', '') or contratacao.get('descricao', '') or contratacao.get('texto_combinado', ''))
                
                # Extrair apenas a data (YYYY-MM-DD) sem timestamp
                data_pub = contratacao.get('data_publicacao_pncp', 'N/A')
                if data_pub != 'N/A' and len(data_pub) >= 10:
                    data_pub = data_pub[:10]  # Pega apenas YYYY-MM-DD
                
                metadata = {
                    "orgao": contratacao.get('nome_orgao_entidade', 'N/A'),
                    "data_publicacao": data_pub,
                    "descricao": descricao_completa[:100]
                }
            ...
            # Inserção em lote com contagem precisa
            insert_query = """
                WITH inserted AS (
...
def partition_contratacoes(contratacoes, num_workers):
    ...
    # Partições menores com workers reduzidos
...
def main():
    """Função principal otimizada do processamento de embeddings"""
    global debug_mode
    ...
    console.print(Panel("[bold blue][4C/7] GERAÇÃO DE EMBEDDINGS OTIMIZADA V1[/bold blue]"))
    
    log_message(f"Arquivo de log criado: {log_filename}", "info")
    log_message(f"Configurações otimizadas: {MAX_WORKERS} workers", "info")
    ...
    try:
        # Verificar/criar constraints de unicidade
        create_embedding_constraints_if_not_exists()
        ...
        log_message(f"Processamento otimizado concluído em {tempo_total:.1f}s", "success")
        ...
        if global_stats['erros'] == 0:
            console.print(Panel("[bold green]✅ PROCESSAMENTO DE EMBEDDINGS OTIMIZADO CONCLUÍDO[/bold green]"))
        else:
