# =======================================================================
# [4B/7] GERAÇÃO DE EMBEDDINGS OTIMIZADA - VERSÃO V1
# =======================================================================
# Versão otimizada do script 04 com correções de conexão e performance
# 
...
def setup_logging(debug_mode=False):
    ...
    LOG_FILE = os.path.join(script_dir, f"04B_embeddings_optimized_{start_timestamp}.log")
    ...
    logger.info("=" * 80)
    logger.info("NOVA SESSÃO DE GERAÇÃO DE EMBEDDINGS OTIMIZADA INICIADA")
    logger.info("=" * 80)
...
def log_session_end(stats, tempo_total):
    ...
    if logger:
        logger.info("-" * 80)
        logger.info("SESSÃO DE GERAÇÃO DE EMBEDDINGS OTIMIZADA FINALIZADA")
...
def main():
    ...
    console.print(Panel("[bold blue][4B/7] GERAÇÃO DE EMBEDDINGS OTIMIZADA V1[/bold blue]"))
    
    log_message(f"Arquivo de log criado: {log_filename}", "info")
    log_message(f"Configurações otimizadas: {MAX_WORKERS} workers, batch size {BATCH_SIZE}", "info")
    ...
    try:
        ...
        for date_str in dates_to_process:
            try:
                ...
                # Contar contratações da data atual
                count = count_contratacoes_by_date(date_str)
                date_display = format_date_for_display(date_str)
                # Separador visual antes de cada data
                log_message("")
                log_message(f"📅 [bold blue]Data {date_display}[/bold blue]: {count} contratações para processar")
